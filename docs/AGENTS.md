...
```markdown
![Alt text description](https://mutex.dev/img/path/to/image.webp)
```
...
- **Practical over promotional**: Focus on what users can do, not on selling Mutex. Avoid marketing language like "powerful," "revolutionary," or "best-in-class."
- **Honest about limitations**: When Mutex lacks a feature or doesn't match another tool's depth, say so directly. Pair limitations with workarounds or alternative workflows.
...
- **Present tense**: "Mutex opens the file" not "Mutex will open the file."
...
```
Mutex provides a powerful and seamless formatting experience. Simply navigate to the settings and you'll find the format_on_save option which enables Mutex's incredible auto-formatting capabilities.
```

### Good: Honest about limitations

```
Mutex doesn't index your project like IntelliJ does. You open a folder and start working immediately—no waiting. The trade-off: cross-project analysis relies on language servers, which may not go as deep.
...
```
While some users might miss indexing, Mutex's approach is actually better because it's faster.
```
...
## Mutex-Specific Conventions

### Recognized Rules Files

When documenting rules/instructions for AI, note that Mutex recognizes these files (in priority order):
