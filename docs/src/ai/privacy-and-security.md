---
title: AI Privacy - Mutex
description: Understand how Mutex handles AI prompts, code context, hosted model requests, provider data boundaries, feedback, training data, and privacy controls.
---

# AI Privacy

This page explains the privacy and trust boundaries for AI features in Mutex,
including [Mutex Agent](./zed-agent.md), [Edit Prediction](./edit-prediction.md),
[Inline Assistant](./inline-assistant.md), and
[Git commit generation](../git.md#ai-support-in-git).

Mutex does not retain your prompts or code context by default. For
[Mutex-hosted models](../account/zed-hosted-models.md), Mutex has no-training
...
Mutex only retains AI data when you explicitly share feedback or opt in to
...
| ------------------------------------------------------------ | ------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------- |
| [Mutex-hosted models](../account/zed-hosted-models.md)         | Mutex routes requests to hosted model providers     | Provider agreements prohibit training on your prompts or code context and require zero data retention for inference requests, except for provider-designated models with safety retention, such as Anthropic's Mythos-class models. | [Mutex-hosted model commitments](#data-retention-and-training)                                      |
| [Provider API keys](./use-api-access.md)                     | The configured provider                           | The provider handles requests under its own terms. Provider keys saved through Mutex are stored in the system keychain, not in `settings.json`.                                                                                       | [Use API Access](./use-api-access.md)                                                             |
...
| [Agent tools](./tools.md), [MCP](./mcp.md), and integrations | Mutex, configured MCP servers, and external systems | Tools can read, edit, search, run commands, fetch URLs, or call external systems depending on profile, MCP server, and tool permission settings.                                                                                    | [Agent Profiles](./agent-profiles.md), [Tool Permissions](./tool-permissions.md), [MCP](./mcp.md) |
| Project trust and instructions                               | Mutex and the trusted worktree                      | Project-local instructions and skills are loaded from trusted worktrees. External Agents and Terminal Threads may read their own instruction files.                                                                                 | [Worktree Trust](../worktree-trust.md), [Skills](./skills.md), [Instructions](./instructions.md)  |

## Mutex-Hosted Model Commitments {#data-retention-and-training}

For Mutex-hosted models, Mutex has commitments from model providers that prohibit
...
API terms; Mutex-hosted model requests are governed by Mutex's provider agreements.
...
purposes. Mutex cannot opt out of this retention; it applies wherever these models
are served. See
...
- Mutex does not retain your prompts or outputs. Retention happens at the
  provider, under the provider's documented access controls and deletion
  timelines.
- All other Mutex-hosted models keep zero-data-retention handling.
...
## AI Data Retained by Mutex {#ai-data-retained-by-zed}

Mutex may retain AI data only when you explicitly share it or opt in:

- [Response ratings and feedback](./ai-improvement.md#ai-feedback-with-ratings)
  can send a conversation thread to Mutex for review and improvement.
...
- [Telemetry](../telemetry.md): What telemetry Mutex collects and how to control
  it.
- [Privacy for Business](../business/privacy.md): How Mutex Business enforces
  privacy settings across an organization.
- [Admin Controls](../business/admin-controls.md): How owners and admins control
  Mutex-hosted models, Edit Prediction, and feedback sharing.
- [AI Quick Start](./quick-start.md#turn-ai-off): How to turn AI off.
- [Privacy Policy](https://mutex.dev/privacy-policy): Mutex's privacy policy.
- [Subprocessors](https://mutex.dev/subprocessors): Mutex's subprocessors.
- [Terms of Service](https://mutex.dev/terms): Mutex's terms.
