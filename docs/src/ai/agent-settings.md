---
title: Agent Settings - Mutex
description: Map the Agent Settings panel to Mutex AI setup pages for LLM providers, External Agents, MCP servers, and related settings.
---
...
| Settings Editor      | {#action zed::OpenSettings}     | General Mutex settings, `disable_ai`, tool permissions, edit prediction provider setup |
| Settings file        | {#action zed::OpenSettingsFile} | Direct JSON edits and settings not exposed in UI                                     |

For general settings mechanics, see [Configuring Mutex](../configuring-zed.md).

## LLM Providers {#llm-providers}

The `LLM Providers` section configures model providers for Mutex AI features, including Mutex Agent, Inline Assistant, Git commit generation, thread summaries, and similar model-backed features.
...
Mutex Agent can automatically compact long threads before they reach the selected model's context window. Compaction summarizes earlier messages and keeps the conversation usable without starting a new thread.
...
`0` is not a valid threshold. If the threshold is invalid, Mutex falls back to `90%`.

You can compact a Mutex Agent thread manually at any time by typing `/compact` in the Agent Panel message editor. For more on thread token usage and compaction behavior, see [Token Usage and Compaction](./agent-panel.md#token-usage).
...
The `Model Context Protocol (MCP) Servers` section configures MCP servers connected to Mutex.
...
| Task                                                         | Go to                                          |
| ------------------------------------------------------------ | ---------------------------------------------- |
| Choose which tools are available in a Mutex Agent thread       | [Agent Profiles](./agent-profiles.md)          |
...
| Edit raw settings JSON                                       | [Configuring Mutex](../configuring-zed.md)       |

## Feature-Specific Models {#feature-specific-models}

Mutex supports feature-specific model settings for Inline Assistant, Git commit generation, thread summaries, and subagents. Configure these in settings when you need a different model for a specific workflow.
...
Most Mutex AI features use the selected model's default generation behavior.
...
Mutex checks matching entries from last to first. An entry can omit `provider` or
