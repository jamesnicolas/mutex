---
title: Feedback and Training Data - Mutex
description: Understand opt-in AI feedback ratings, Edit Prediction training data, and when Mutex may retain AI data.
---

# Feedback and Training Data

Normal AI requests are not retained by Mutex. For
[Mutex-hosted models](../account/zed-hosted-models.md), provider agreements
...
This page covers the cases where Mutex may retain AI data because you explicitly
shared it or opted in.

AI features in Mutex include:
...
Mutex-hosted model zero-data-retention and no-training commitments, including the
...
You can rate AI responses or submit feedback to help improve Mutex's system prompt,
...
> If you do not want the thread persisted by Mutex, do not rate the response.

### Data Collected from Feedback {#data-collected-ai-feedback}

For conversation threads you explicitly share through ratings or feedback, Mutex
may store:
...
- metadata about your Mutex installation

If you do not rate responses or submit feedback, Mutex does not store Customer Data
related to your AI feature usage for improvement.

Telemetry related to Mutex's AI features is collected separately. This includes
...
Collected feedback data is stored in Snowflake. Mutex periodically reviews this
...
Mutex does not collect training data for the Edit Prediction model unless all of
...
[`edit_predictions.disabled_globs`](https://mutex.dev/docs/reference/all-settings#edit-predictions)
in your Mutex settings file ([how to edit](../configuring-zed.md#settings-files)):
...
For open source projects where you opted in, Mutex may collect:
...
Collected data is stored in Snowflake. Mutex periodically reviews this data to
...
Mutex uses this training dataset to fine-tune
...
On Mutex Business, data sharing is off by default and controlled by organization
...
See the [Mutex Terms of Service](https://mutex.dev/terms) for more.
