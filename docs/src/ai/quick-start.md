---
title: AI Quick Start - Mutex
description: Choose the right Mutex AI setup path for agents, models, subscriptions, local models, edit prediction, and privacy.
---

# AI Quick Start

Use this page to choose the right AI setup path in Mutex. If you already know the company, subscription, model provider, agent, or CLI you want to use, start with [AI by Company](./by-company.md).

## Use the Mutex Agent {#zed-agent}

Use [Mutex Agent](./zed-agent.md) when you want Mutex's native agent to read, edit, search, and run code in your project.

Mutex Agent uses Mutex-configured models from [LLM Providers](./llm-providers.md). It also uses Mutex's built-in tools, [Agent Profiles](./agent-profiles.md), [Skills](./skills.md), [Instructions](./instructions.md), and [MCP servers](./mcp.md).

Start in the [Agent Panel](./agent-panel.md) to prompt the agent, add context, review changes, and manage threads.

### Configure the Mutex Agent {#configure-zed-agent}
...
## Use Another Coding Agent in Mutex {#agent-cli}

Use this path for Claude, Codex, OpenCode, Copilot, Cursor, Pi Coding Agent, Gemini CLI, or another coding agent.

| If the agent...                 | Use                                       |
| ------------------------------- | ----------------------------------------- |
| Integrates with Mutex through ACP | [External Agents](./external-agents.md)   |
...
## Choose Which Models the Mutex Agent Uses {#model-access}

The Mutex Agent and other model-backed Mutex AI features use models configured through [LLM Providers](./llm-providers.md).

| If you want to...                                                             | Use                                                               |
| ----------------------------------------------------------------------------- | ----------------------------------------------------------------- |
| Use models billed through Mutex                                                 | [Mutex-Hosted Models](../account/zed-hosted-models.md)              |
...
Provider keys saved through Mutex are stored in the system keychain, not in `settings.json`.
...
| Understand plans, usage, and billing | [Plans & Pricing](../account/plans-and-pricing.md), [Mutex-Hosted Models](../account/zed-hosted-models.md), and [Billing](../account/billing.md) |
