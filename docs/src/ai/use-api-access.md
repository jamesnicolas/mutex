---
title: Use API Access - Mutex
description: Configure provider API access, API keys, API credits, usage billing, and OpenAI-compatible endpoints for Mutex AI features.
---
...
Mutex supports these first-class API providers for model-backed Mutex AI features:
...
Use API access for the Mutex Agent, Inline Assistant, Git commit generation,
thread summaries, and similar Mutex-owned AI features.
...
Most API-access providers can be configured in Mutex's Agent Settings panel with {#action agent::OpenSettings}. Keys saved through Mutex are stored in the system keychain, not in `settings.json`.

Mutex also reads provider-specific environment variables. Non-empty environment variables take precedence over keychain values. If a key comes from an environment variable, unset the variable and restart Mutex to stop using it.
...
You can attach extra HTTP headers to every request Mutex makes to supported HTTP-based providers. This is useful in corporate environments or for observability tooling.
...
Headers managed by Mutex for each provider, such as `Authorization`, `Content-Type`, `Accept`, and provider-specific authentication headers, are ignored with a warning if you try to override them.

## Remote Projects {#remote-projects}

Mutex LLM providers for Mutex AI features are initialized in the local Mutex app. In SSH, dev container, and other remote projects, API keys saved in Mutex are read from the local system keychain, and provider environment variables are read from the local Mutex process environment.
...
Mutex also reads `ANTHROPIC_API_KEY` from the local Mutex process environment.
...
Mutex also reads `OPENAI_API_KEY` from the local Mutex process environment.
...
Mutex reads `GEMINI_API_KEY`, falling back to `GOOGLE_AI_API_KEY`, from the local Mutex process environment.
...
Mutex also reads `MISTRAL_API_KEY` from the local Mutex process environment.
...
Use DeepSeek API access when you have paid API usage, top-ups, or an API key. In Mutex, DeepSeek is API access, not subscription sign-in.
...
Mutex also reads `DEEPSEEK_API_KEY` from the local Mutex process environment.
...
Mutex also reads `XAI_API_KEY` from the local Mutex process environment.
...
Mutex does not sign in to OpenCode with OAuth or detect your OpenCode subscription; it uses an OpenCode API key saved in the system keychain or `OPENCODE_API_KEY`.
...
Mutex also reads `OPENCODE_API_KEY` from the local Mutex process environment.
...
**Note:** Mutex only bundles configuration for long-term OpenCode Free models. Free models that are available for a limited time are not included in Mutex. To use those models, add a custom OpenCode model with configuration from [the OpenCode website](https://opencode.ai/docs/zen#pricing) and [models.dev](https://github.com/anomalyco/models.dev/tree/dev/providers/opencode/models).

#### Custom OpenCode Models {#opencode-custom-models}

The Mutex Agent comes preconfigured with OpenCode models. Add custom OpenCode models when you need newer models, limited-time Free models, or models with custom endpoints.
...
The optional `custom_headers` map adds extra headers to every request, which some providers require. Headers managed by Mutex (such as `X-Api-Key` and `Anthropic-Version`) cannot be overridden.
...
If a model only works with the Responses API, set `capabilities.chat_completions` to `false`. Mutex will use the Responses endpoint for that model.
