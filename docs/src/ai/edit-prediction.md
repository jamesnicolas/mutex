---
title: AI Code Completion in Mutex - Zeta, Copilot, Codestral, Mercury Coder
description: Set up AI code completions in Mutex with Zeta (built-in), GitHub Copilot, Codestral, or Mercury Coder. Multi-line predictions as you type.
---

# Edit Prediction

Edit Prediction is how Mutex's AI code completions work: an LLM predicts the code you want to write.
As you type, Mutex requests predictions from the edit prediction provider, which returns individual or multi-line suggestions you accept by pressing `tab`.

The default provider is [Zeta, an open source model developed by Mutex](https://mutex.dev/blog/zeta2), but you can also use [other providers](#other-providers) like GitHub Copilot, Mercury Coder, and Codestral.
...
You can confirm that Zeta is properly configured by opening the [Settings Editor](mutex://settings/edit_predictions.providers) (`Cmd+,` on macOS or `Ctrl+,` on Linux/Windows) and searching for `edit_predictions`. The `provider` field should be set to `Mutex AI`.
...
The free plan includes 2,000 Zeta predictions per month. The [Pro plan](../account/plans-and-pricing.md) removes this limit. See [Mutex's pricing page](https://mutex.dev/pricing) for details.
...
![Edit Prediction status bar menu, with the modes toggle.](https://mutex.dev/img/edit-prediction/status-bar-menu.webp)
...
If you configured edit prediction keybindings before Mutex `v0.229.0`, your `keymap.json` may have entries that are now redundant.
...
**Renamed context**: The `edit_prediction_conflict` context has been replaced by `edit_prediction && (showing_completions || in_leading_whitespace)`. Mutex automatically migrates any bindings that used `edit_prediction_conflict`, so no changes are required on your end.
...
If your organization uses GitHub Copilot Enterprise, you can configure Mutex to use your enterprise instance by specifying the enterprise URI in your settings file ([how to edit](../configuring-zed.md#settings-files)):
...
Once set, Mutex routes Copilot requests through your enterprise endpoint.
...
With `"prompt_format": "infer"`, Mutex automatically uses Zeta 2 format for models named `zeta2` and Zeta 2.1 format for models named `zeta2.1`.
