...
To restore a thread, open Thread History and click the thread you want to bring back. Mutex moves it back to the thread list and opens it in the Agent Panel. If the thread was running in a Git worktree that was removed, Mutex restores the worktree automatically.
...
Each thread can use a different agent, so you can run Mutex's built-in agent in one thread and an [External Agent](./external-agents.md) like Claude Code or Codex in another.
...
| --------------------------------------------- | ------------------------------------------------------------------------------- |
| [Mutex Agent thread](./zed-agent.md)            | Uses Mutex Agent settings, profiles, tools, Skills, Instructions, and MCP         |
...
The Threads Sidebar can hold multiple projects at once. Each project gets its own group with its own threads and conversation history. This mirrors how Mutex handles projects in general — see [Windows & Projects](../windows-and-projects.md) for more on how projects open and how to manage them.
