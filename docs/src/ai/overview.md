---
title: AI in Mutex
description: Understand Mutex's AI features, agent paths, model providers, and setup routes.
---

# AI

Mutex's AI docs are organized around three areas:

| Area         | Use it to choose                    | Examples                                                             |
| ------------ | ----------------------------------- | -------------------------------------------------------------------- |
| Agents       | How agentic work runs in Mutex        | Mutex Agent, External Agents, Terminal Threads                         |
| Model access | How Mutex connects to language models | Mutex-hosted models, API access, subscriptions, gateways, local models |
...
Agent paths decide how agentic work runs in Mutex.

- [Mutex Agent](./zed-agent.md): Mutex's native agent. It can use models configured through [LLM Providers](./llm-providers.md), including Mutex-hosted models, provider API keys, supported subscriptions, gateways, and local models. It also uses built-in tools, profiles, skills, instructions, and MCP servers.
- [External Agents](./external-agents.md): ACP-integrated agents that run through their own process and configuration.
- [Terminal Threads](./terminal-threads.md): terminal-backed threads for running an agent CLI or TUI directly in Mutex.
...
Model access controls which models power the Mutex Agent and other model-backed Mutex AI features. Mutex can use hosted models, provider API access, subscription sign-in, gateways, and local models.

See [LLM Providers](./llm-providers.md) to choose a model access path.

## AI Features {#ai-features}

Mutex has several AI-powered workflows:
