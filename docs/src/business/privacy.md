---
title: Privacy for Business - Mutex Business
description: How Mutex Business handles data privacy across your organization, including enforced protections for prompts and training data.
---

# Privacy for Business

Mutex Business removes the per-member data-sharing options that Free and Pro
...
For all members of a Mutex Business organization:

- **No prompt sharing:** Conversations and prompts are never shared with Mutex.
...
- **No training data sharing:** Code context is never shared with Mutex for
...
- Disable Mutex-hosted models entirely via the Mutex Model Provider toggle, so no
  prompts reach Mutex's infrastructure
