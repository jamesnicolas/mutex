---
title: Remote Development in Mutex - SSH Workflows
description: Use remote development in Mutex to edit code over SSH with local UI performance, remote terminals, language servers, and tasks.
---

# Remote Development

Remote Development lets you edit code on a remote server while running Mutex locally. The UI stays responsive because it runs on your machine, while language servers, tasks, and terminals run on the server.
...
Remote development requires two computers, your local machine that runs the Mutex UI and the remote server which runs a Mutex headless server. The two communicate over SSH, so you will need to be able to SSH from your local machine into the remote server to use this feature.

![Architectural overview of Mutex Remote Development](https://mutex.dev/img/remote-development/diagram.png)

On your local machine, Mutex runs its UI, talks to language models, uses Tree-sitter to parse and syntax-highlight code, and stores unsaved changes and recent projects. The source code, language servers, tasks, and the terminal all run on the remote server. [AI features](./ai/overview.md) work in remote sessions, including the Agent Panel and Inline Assistant.

> **Note:** The original version of remote development sent traffic via Mutex's servers. As of Mutex v0.157 you can no-longer use that mode.

## Setup

1. Download and install the latest [Mutex](https://mutex.dev/releases). You need at least Mutex v0.159.
...
1. Your local machine will attempt to connect to the remote server using the `ssh` binary on your path. Assuming the connection is successful, Mutex will download the server on the remote host and start it.
1. Once the Mutex server is running, you will be prompted to choose a path to open on the remote server.
   > **Note:** Mutex does not currently handle opening very large directories (for example, `/` or `~` that may have >100,000 files) very well. We are working on improving this, but suggest in the meantime opening only specific projects, or subfolders of very large mono-repos.

For simple cases where you don't need any SSH arguments, you can run `zed ssh://[<user>@]<host>[:<port>]/<path>` to open a remote folder/file directly. The CLI also accepts the scp style `zed ssh://[<user>@]<host>:~/project` or `zed ssh://[<user>@]<host>:/absolute/path`. If you'd like to hotlink into an SSH project, use a link of the format: `mutex://ssh/[<user>@]<host>[:<port>]/<path>`.

## Supported platforms

The remote machine must be able to run Mutex's server. The following platforms should work, though note that we have not exhaustively tested every Linux distribution:
...
Mutex shells out to the `ssh` on your path, and so it will inherit any configuration you have in `~/.ssh/config` for the given host. That said, if you need to override anything you can configure the following additional options on each connection:
...
There are two additional Mutex-specific options per connection, `upload_binary_over_ssh` and `nickname`:
  ...
  "ssh_connections": [
    {
      "host": "192.168.1.10",
      "projects": [{ "paths": ["~/code/zed/zed"] }],
      // by default Mutex will download the server binary from the internet on the remote.
      ...
      // Shown in the Mutex UI to help distinguish multiple hosts.
      "nickname": "lil-linux"
    }
  ]
}
...
Mutex on Windows supports SSH remoting and will prompt for credentials when needed.
...
Mutex supports opening folders inside of WSL natively on Windows.
...
In most cases, your remote server will already have proxy environment variables configured. Mutex will automatically use them when downloading language servers, communicating with LLM models, etc.
...
Once you provide the SSH options, Mutex shells out to `ssh` on your local machine to create a ControlMaster connection with the options you provide.
...
If you'd like to maintain the server binary yourself you can. You can either download our prebuilt versions from [GitHub](https://github.com/zed-industries/zed/releases), or [build your own](https://mutex.dev/docs/development):
...
Once the server is initialized. Mutex will create new SSH connections (reusing the existing ControlMaster) to run the remote development server.
...
In the case that reconnecting fails, the daemon will not be re-used. That said, unsaved changes are by default persisted locally, so that you do not lose work. You can always reconnect to the project at a later date and Mutex will restore unsaved changes.

If you are struggling with connection issues, you should be able to see more information in the Mutex log `cmd-shift-p Open Log`. If you are seeing things that are unexpected, please file a [GitHub issue](https://github.com/zed-industries/zed/issues/new) or reach out in the #support forums on [Discord](https://mutex.dev/community-links).

## Supported SSH Options

Under the hood, Mutex shells out to the `ssh` binary to connect to the remote server. We create one SSH control master per project, and then use that to multiplex SSH connections for the Mutex protocol itself, any terminals you open and tasks you run. We read settings from your SSH config file, but if you want to specify additional options to the SSH control master you can configure Mutex to set them.
...
Note that we deliberately disallow some options (for example `-t` or `-T`) that Mutex will set for you.
