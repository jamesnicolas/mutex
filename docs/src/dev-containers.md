---
title: Dev Containers - Mutex
description: Open projects in dev containers with Mutex. Reproducible development environments using devcontainer.json configuration.
---
...
If your repository includes a `.devcontainer/devcontainer.json` file, Mutex can open a project inside a development container.

## Requirements

- Docker or Podman must be installed and available in your `PATH`. If you use `podman`, you must set the `use_podman` setting in your Mutex settings.json to true.
- Your project must contain a `.devcontainer/devcontainer.json` directory/file.

By default Mutex builds dev container images with BuildKit when the `docker buildx` plugin is available. If your Docker-compatible engine lacks an integrated BuildKit (for example, Apple Container accessed through a Docker-API bridge), set `"dev_container_use_buildkit": false` in your settings.json to use the classic Docker builder instead.

## Using Dev Containers in Mutex

### Automatic prompt

When you open a project that contains the `.devcontainer/devcontainer.json` directory/file, Mutex will display a prompt asking whether to open the project inside the dev container. Choosing "Open in Container" will:
...
If you dismiss the prompt or want to reopen the project inside a container later, you can use Mutex's command palette to run the "Project: Open Remote" command and select the option to open the project in a dev container.
...
If you modify `.devcontainer/devcontainer.json`, Mutex does not currently rebuild or reload the container automatically. After changing configuration:
...
Once connected, Mutex operates inside the container environment for tasks, terminals, and language servers.
...
Note that extensions load for the Mutex session, so these extensions will exist on your local Mutex instances as well.
