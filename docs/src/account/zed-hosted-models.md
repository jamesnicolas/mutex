---
title: Mutex-Hosted Models - Mutex
description: AI models available via Mutex Pro including Claude, GPT-5.5, Gemini 3.1 Pro, and Gemini 3.5 Flash. Pricing, context windows, and tool call support.
---

# Mutex-Hosted Models

Mutex's plans offer hosted versions of major LLMs with higher rate limits than direct API access. Model availability is updated regularly. To use your own API keys instead, see [LLM Providers](../ai/llm-providers.md). For general setup, see [AI Quick Start](../ai/quick-start.md).

> **Note:** Claude Opus models, GPT-5.5 pro, and GPT-5.4 pro are only available on Mutex Pro and Mutex Business.

| Model             | Provider  | Token Type          | Provider Price per 1M tokens | Mutex Price per 1M tokens |
| ----------------- | --------- | ------------------- | ---------------------------- | ----------------------- |
...
As of February 19, 2026, Mutex Pro serves newer model versions in place of the retired models below:
...
| Model             | Provider  | Mutex-Hosted Context Window |
| ----------------- | --------- | ------------------------- |
...
> Mutex currently limits hosted Gemini 3.1 Pro requests to 200k tokens because pricing changes above that context size.

Each Agent thread in Mutex maintains its own context window.
