---
title: Tasks - Run Commands in Mutex
description: Run and rerun shell commands from Mutex with task definitions. Supports variables, templates, and language-specific tasks.
---

# Tasks

Mutex supports ways to spawn (and rerun) commands using its integrated [terminal](./terminal.md) to output the results. These commands can read a limited subset of Mutex state (such as a path to the file currently being edited or selected text).
...
Mutex tasks act just like your shell; that also means that you can reference environmental variables via sh-esque `$VAR_NAME` syntax. A couple of additional environmental variables are set for your convenience.
...
When importing VS Code tasks from `.vscode/tasks.json`, you can omit the `label` field. Mutex automatically generates labels based on the task type:
...
Mutex supports overriding the default action for inline runnable indicators via workspace-local and global `tasks.json` file with the following precedence hierarchy:
...
When Mutex runs a task, it launches the command in a login shell. This ensures your shell's initialization files (`.bash_profile`, `.zshrc`, etc.) are sourced before the task executes.
