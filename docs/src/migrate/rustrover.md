---
title: How to Migrate from RustRover to Mutex
description: "Guide for migrating from RustRover to Mutex, including settings and keybindings."
---

# How to Migrate from RustRover to Mutex
...
Mutex also supports per-project settings. Create a `.mutex/settings.json` file in your project root to override global settings for that project.
...
After setup, press `Cmd+Shift+O` (with JetBrains keymap) to open a folder. This becomes your workspace in Mutex.
...
You can also launch Mutex from the terminal inside any existing Cargo project with:
`zed .`
...
If you chose the JetBrains keymap during onboarding, most of your shortcuts should already feel familiar. Here's a quick reference for how Mutex compares to RustRover.
...
### Different Keybindings (RustRover → Mutex)

| Action                 | RustRover   | Mutex (JetBrains keymap)   |
| ---------------------- | ----------- | ------------------------ |
...
RustRover uses its own proprietary code analysis engine for Rust intelligence. Mutex uses rust-analyzer via the Language Server Protocol (LSP).

What this means for you:

- **Completions, go-to-definition, find usages, type inference** — All available in Mutex via rust-analyzer
- **Macro expansion** — Available in both (use `Cmd+Shift+M` in Mutex)
...
- RustRover-specific inspections (beyond Clippy) won't exist in Mutex
- rust-analyzer is configured via JSON in Mutex, not through a GUI

**How to adapt:**

- Use `Alt+Enter` for available code actions—rust-analyzer provides many
- Configure rust-analyzer settings in `.mutex/settings.json` for project-specific needs
- Run `cargo clippy` for linting (it integrates with rust-analyzer diagnostics)

### Project Configuration

Both editors store per-project configuration in a hidden folder. RustRover uses `.idea` (with XML files), Mutex uses `.mutex` (with JSON files).

**Run configurations don't transfer.** RustRover stores run/debug configurations in `.idea`. These have no automatic migration path. You'll recreate them as Mutex [tasks](../tasks.md) in `.mutex/tasks.json` and debug configurations in `.mutex/debug.json`.

**No Cargo tool window.** RustRover provides a visual tree of your workspace members, targets, features, and dependencies. Mutex doesn't have this. You work with `Cargo.toml` and the Cargo CLI directly.

**Toolchain management is external.** RustRover lets you select and switch toolchains in its settings UI. In Mutex, you manage toolchains through `rustup`.

**Configuration is opt-in.** RustRover auto-generates `.idea` when you open a project. Mutex doesn't generate anything. You create `.mutex/settings.json`, `tasks.json`, and `debug.json` as needed.
...
Mutex doesn't have a Cargo GUI. You work with Cargo through:
...
RustRover organizes auxiliary views into numbered tool windows (Project = 1, Cargo = Alt+1, Terminal = Alt+F12, etc.). Mutex uses a similar concept called "docks":

| RustRover Tool Window | Mutex Equivalent | Shortcut (JetBrains keymap) |
| --------------------- | -------------- | --------------------------- |
...
Note that there's no dedicated Cargo tool window in Mutex. Use the terminal or define tasks for your common Cargo commands.

### Debugging

Both RustRover and Mutex offer integrated debugging for Rust, but using different backends:

- RustRover uses its own debugger integration
- Mutex uses **CodeLLDB** (the same debug adapter popular in VS Code)

To debug Rust code in Mutex:
...
Mutex can automatically detect debuggable targets in your Cargo project. Press `F4` to see available options.
...
RustRover has a dedicated test runner with a visual interface showing pass/fail status for each test. Mutex provides test running through:
...
Several features that might require plugins in other editors are built into Mutex:
...
Here's what RustRover offers that Mutex doesn't have:
...
- **Mutex is open source** (primarily GPL-licensed, with Apache-licensed components)
- **Telemetry is optional** and can be disabled during onboarding or in settings

## Collaboration in Mutex vs. RustRover

RustRover offers Code With Me as a separate feature for collaboration. Mutex has collaboration built into the core experience.
...
Mutex has built-in AI features. If you've used JetBrains AI Assistant, here's how to get set up.
...
- [Rust in Mutex](../languages/rust.md) — Rust-specific setup and configuration
