---
title: How to Migrate from VS Code to Mutex
description: "Guide for migrating from VS Code to Mutex, including settings and keybindings."
---

# How to Migrate from VS Code to Mutex

This guide explains how to move from VS Code to Mutex without rebuilding your workflow.
...
After installation, you can launch Mutex from your Applications folder (macOS) or directly from the terminal (Linux) using:
`zed .`
This opens the current directory in Mutex.

## Import Settings from VS Code

During setup, you have the option to import key settings from VS Code. Mutex imports the following settings:
...
Mutex doesn’t import extensions or keybindings, but this import gets core editor behavior close to your VS Code setup. If you skip that step during setup, you can still import settings manually later via the command palette:
...
| VS Code | Mutex | Notes |
| --- | --- | --- |
| editor.fontFamily | buffer_font_family | Mutex uses Mutex Mono by default |
...
Mutex also supports per-project settings. You can find these in the Settings Editor as well.
...
### Common Shared Keybindings (Mutex <> VS Code)
...
### Different Keybindings (Mutex <> VS Code)

| Action              | VS Code               | Mutex                    |
| ------------------- | --------------------- | ---------------------- |
...
Mutex also supports chords (multi-key sequences) like `Cmd+K Cmd+C`, like VS Code does.
...
Mutex takes a different approach.
...
- **Per-project settings are optional**: You can add a `.mutex/settings.json` file inside a project to override global settings.

- **You can start from a single file or an empty window**: Mutex doesn’t require you to open a folder to begin editing.
...
- Instead of a persistent panel, Mutex encourages you to:
...
Mutex does not offer as many extensions as VS Code. The available extensions are focused on language support, themes, syntax highlighting, and other core editing enhancements.

Several features that typically require extensions in VS Code are built into Mutex:
...
You won’t find one-to-one replacements for every VS Code extension, especially if you rely on tools for DevOps, containers, or test runners. Mutex's extension catalog is still growing and remains smaller.

### Collaboration in Mutex vs. VS Code

Unlike VS Code, Mutex doesn’t require an extension to collaborate. It’s built into the core experience.
...
Learn how [Mutex uses Mutex](https://mutex.dev/blog/zed-is-our-office) to plan work and collaborate.

### Using AI in Mutex

If you’re used to GitHub Copilot in VS Code, you can do the same in Mutex. You can also explore other agents through Mutex Pro, or bring your own keys and connect without authentication. You can disable AI features entirely if you prefer.
...
Copy your VS Code snippet JSON directly into Mutex's snippets folder ({#action snippets::ConfigureSnippets}).
