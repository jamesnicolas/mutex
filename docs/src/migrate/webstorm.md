---
title: How to Migrate from WebStorm to Mutex
description: "Guide for migrating from WebStorm to Mutex, including settings and keybindings."
---

# How to Migrate from WebStorm to Mutex

This guide covers how to set up Mutex if you're coming from WebStorm, including keybindings, settings, and the differences you should expect as a JavaScript/TypeScript developer.
...
Mutex also supports per-project settings. Create a `.mutex/settings.json` file in your project root to override global settings for that project, similar to how you might use `.idea` folders in WebStorm.
...
After setup, use {#kb:jetbrains file_finder::Toggle} to open a folder. This becomes your workspace in Mutex. Unlike WebStorm, there's no project configuration wizard, no framework selection dialog, and no project structure setup required.

To start a new project, create a directory using your terminal or file manager, then open it in Mutex. The editor will treat that folder as the root of your project. For new projects, you'd typically run `npm init`, `pnpm create`, or your framework's CLI tool first, then open the resulting folder in Mutex.
...
Mutex doesn't index. You open a folder and start coding immediately—no progress bars, no "Indexing paused" banners. File search and navigation stay fast regardless of project size or how many `node_modules` dependencies you have.

WebStorm's index enables features like finding all usages across your entire codebase, tracking import hierarchies, and flagging unused exports project-wide. Mutex relies on language servers for this analysis, which may not cover as much ground.
...
Mutex uses the Language Server Protocol (LSP) for code intelligence. For JavaScript and TypeScript, Mutex supports:
...
Mutex takes a different approach: a project is just a folder. There's no setup wizard, no framework detection dialog, no project structure to configure.
...
- No framework detection. Mutex treats React, Angular, Vue, and vanilla JS/TS the same way.
...
Mutex has none of this built-in. The TypeScript language server sees your code as TypeScript—it doesn't understand that a function is a React component or that a file is an Angular service.
...
- Consider using framework-specific CLI tools (`ng`, `next`, `vite`) from Mutex's terminal
- For React, JSX/TSX syntax and TypeScript types still provide good intelligence

> **Tip:** For projects with complex configurations, keep your framework's documentation handy. Mutex's speed comes with less hand-holding for framework-specific features.

### Tool Windows vs. Docks

WebStorm organizes auxiliary views into numbered tool windows. Mutex uses a similar concept called "docks":

| WebStorm Tool Window | Mutex Equivalent | Mutex Keybinding                             |
| -------------------- | -------------- | ------------------------------------------ |
...
Note that there's no dedicated npm tool window in Mutex. Use the terminal or define tasks for your common npm scripts.

### Debugging

Both WebStorm and Mutex offer integrated debugging for JavaScript and TypeScript:

- Mutex uses `vscode-js-debug` (the same debug adapter that VS Code uses)
...
Mutex also recognizes `.vscode/launch.json` configurations, so existing VS Code debug setups often work out of the box.

### Running Tests

WebStorm has a dedicated test runner with a visual interface showing pass/fail status for each test. Mutex provides test running through:
...
Mutex supports auto-detection for common test frameworks:
...
Several features that require plugins in WebStorm are built into Mutex:
...
To set expectations clearly, here's what WebStorm offers that Mutex doesn't have:
...
## Collaboration in Mutex vs. WebStorm

WebStorm offers Code With Me as a separate feature for collaboration. Mutex has collaboration built into the core experience.
...
If you're used to AI assistants in WebStorm (like GitHub Copilot, JetBrains AI Assistant, or Junie), Mutex offers similar capabilities with more flexibility.
...
- [JavaScript in Mutex](../languages/javascript.md) — JavaScript-specific setup and configuration
- [TypeScript in Mutex](../languages/typescript.md) — TypeScript-specific setup and configuration
