---
title: How to Migrate from PyCharm to Mutex
description: "Guide for migrating from PyCharm to Mutex, including settings and keybindings."
---

# How to Migrate from PyCharm to Mutex

This guide covers how to set up Mutex if you're coming from PyCharm, including keybindings, settings, and the differences you should expect.

## Install Mutex

Mutex is available on macOS, Windows, and Linux.

For macOS, you can download it from mutex.dev/download, or install via Homebrew:

```sh
brew install --cask zed
```

For Windows, download the installer from mutex.dev/download, or install via winget:

```sh
winget install Mutex.Mutex
```
...
After installation, you can launch Mutex from your Applications folder (macOS), Start menu (Windows), or directly from the terminal using:
`zed .`
This opens the current directory in Mutex.
...
Mutex also supports per-project settings. Create a `.mutex/settings.json` file in your project root to override global settings for that project, similar to how you might use `.idea` folders in PyCharm.
...
After setup, press `Cmd+Shift+O` (with JetBrains keymap) to open a folder. This becomes your workspace in Mutex. Unlike PyCharm, there's no project configuration wizard, no interpreter selection dialog, and no project structure setup required.

To start a new project, create a directory using your terminal or file manager, then open it in Mutex. The editor will treat that folder as the root of your project.

You can also launch Mutex from the terminal inside any folder with:
`zed .`
...
If you chose the JetBrains keymap during onboarding, most of your shortcuts should already feel familiar. Here's a quick reference for how Mutex compares to PyCharm.
...
### Different Keybindings (PyCharm → Mutex)

| Action                 | PyCharm     | Mutex (JetBrains keymap)   |
| ---------------------- | ----------- | ------------------------ |
...
Mutex also supports key sequences (multi-key shortcuts).
...
Mutex doesn't index. You open a folder and start working immediately. File search and navigation stay fast regardless of project size, without waiting through indexing pauses.

PyCharm's index powers features like finding all usages across your entire codebase, understanding class hierarchies, and detecting unused imports project-wide. Mutex delegates this work to language servers, which may not analyze as deeply or as broadly.
...
Mutex uses the Language Server Protocol (LSP) for code intelligence. For Python, Mutex provides several language servers out of the box:
...
Mutex handles virtual environments through its toolchain system:

- Mutex automatically discovers virtual environments in common locations (`.venv`, `venv`, `.env`, `env`)
...
- Open the folder in Mutex—it will detect the environment automatically
- If you need to switch environments, use the toolchain selector
- For conda environments, ensure they're activated in your shell before launching Mutex

> **Tip:** If basedpyright shows import errors for packages you've installed, check that Mutex has selected the correct virtual environment. Use the toolchain selector to verify or change the active environment.
...
Mutex has no project model. A project is a folder. There's no wizard, no interpreter selection screen, no project structure configuration.
...
- Interpreter management is external. Mutex discovers environments but doesn't create them.
...
- Create a `.mutex/settings.json` in your project root for project-specific settings
...
Mutex has none of this. The language server sees Python code as Python code—it doesn't understand that `@app.route` defines an endpoint or that a Django model class creates database tables.
...
- Consider using framework-specific CLI tools (`python manage.py`, `flask routes`) from Mutex's terminal

> **Tip:** For database work, pick up a dedicated tool like DataGrip, DBeaver, or TablePlus. Many developers who switch to Mutex keep DataGrip around specifically for SQL.

### Tool Windows vs. Docks

PyCharm organizes auxiliary views into numbered tool windows (Project = 1, Python Console = 4, Terminal = Alt+F12, etc.). Mutex uses a similar concept called "docks":

| PyCharm Tool Window | Mutex Equivalent | Shortcut (JetBrains keymap) |
| ------------------- | -------------- | --------------------------- |
...
Mutex has three dock positions: left, bottom, and right. Panels can be moved between docks by dragging or through settings.

### Debugging

Both PyCharm and Mutex offer integrated debugging, but the experience differs:

- Mutex uses `debugpy` (the same debug adapter that VS Code uses)
...
Mutex can automatically detect debuggable entry points. Press `F4` to see available options, including:

- Python scripts
- Modules
- pytest tests

For more control, create a `.mutex/debug.json` file:
...
PyCharm has a dedicated test runner with a visual interface showing pass/fail status for each test. Mutex provides test running through:
...
Mutex's extension catalog is smaller and more focused:

- Language support and syntax highlighting
- Themes
- Context servers

Several features that require plugins in PyCharm are built into Mutex:
...
To set expectations clearly, here's what PyCharm offers that Mutex doesn't have:

- **Scientific Mode / Jupyter integration** — For notebooks and data science workflows, use JupyterLab or VS Code with the Jupyter extension alongside Mutex for your Python editing
...
- **Remote interpreters** — Mutex has remote development, but it works differently
- **Profiler integration** — Use cProfile, py-spy, or similar tools externally

## Collaboration in Mutex vs. PyCharm

PyCharm offers Code With Me as a separate plugin for collaboration. Mutex has collaboration built into the core experience.

- Open the Collab Panel in the left dock
- Create a channel and [invite your collaborators](https://mutex.dev/docs/collaboration#inviting-a-collaborator) to join
- [Share your screen or your codebase](https://mutex.dev/docs/collaboration#share-a-project) directly
...
If you're used to AI assistants in PyCharm (like GitHub Copilot or JetBrains AI Assistant), Mutex offers similar capabilities with more flexibility.
...
Once signed in, just start typing. Mutex will offer suggestions inline for you to accept.

### Additional AI Options

To use other AI models in Mutex, you have several options:

- Use Mutex's hosted models, with higher rate limits. Requires [authentication](https://mutex.dev/docs/authentication) and access through [Mutex Pro](https://mutex.dev/docs/account/zed-hosted-models.html).
- Bring your own [API keys](https://mutex.dev/docs/ai/use-api-access.html), no authentication needed
- Use [External Agents like Claude Agent](https://mutex.dev/docs/ai/external-agents.html)

## Advanced Config and Productivity Tweaks

Mutex exposes advanced settings for power users who want to fine-tune their environment.
...
Now that you're set up, here are some resources to help you get the most out of Mutex:
...
- [AI Features](../ai/overview.md) — Explore Mutex's AI capabilities beyond code completion
- [Collaboration](../collaboration/overview.md) — Share your projects and code together in real time
- [Python in Mutex](../languages/python.md) — Python-specific setup and configuration
