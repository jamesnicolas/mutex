---
title: Themes - Mutex
description: Browse, install, and create themes for Mutex. Includes built-in themes and community theme extensions.
---

# Themes

Mutex comes with a number of built-in themes, with more themes available as extensions.
...
You can find hundreds of different theme options in Mutex's extensions store, which you can access via the command palette with {#action zed::Extensions} or the [Mutex website](https://mutex.dev/extensions?filter=themes).

Many popular themes have been ported to Mutex, and if you're struggling to choose one, visit [zed-themes.com](https://zed-themes.com), a third-party gallery with visible previews for many of them.

## Build Your Theme

You can use [Mutex's Theme Builder](https://mutex.dev/theme-builder) to design your own custom theme based on an existing one.

This tool lets you fine-tune and preview how every surface in the Mutex app will look.
You can then export the JSON for [local use](./themes.md#local-themes) or for [publishing in Mutex's extension store](./extensions/themes.md).
...
By default, Mutex maintains two themes: one for light mode and one for dark mode.
...
Store new themes locally by placing them in the `~/.config/mutex/themes` directory (macOS and Linux) or `%USERPROFILE%\AppData\Roaming\Mutex\themes\` (Windows).

For example, to create a new theme called `my-cool-theme`, create a file called `my-cool-theme.json` in that directory.
It will be available in the theme selector the next time Mutex loads.
