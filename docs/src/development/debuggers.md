...
description: "Guide to using a debugger for Mutex development."
---

# Using a debugger

> This page is not about [configuring Mutex's debugger](../debugger.md).
> It covers how to use a debugger while developing Mutex itself.

## Using Mutex's built-in debugger

While the Mutex project is open you can open the `New Process Modal` and select the `Debug` tab. There you can see two debug configurations to debug Mutex with, one for GDB and one for LLDB. Select the configuration you want and Mutex will build and launch the binary.
...
To get the full debugger experience on a release build, compile a Mutex binary with full debug info.
...
## Running Mutex with a shell debugger GDB/LLDB

### Background

When you install Rust through rustup (the recommended setup for Mutex development; see your platform guide [here](../development.md)), rustup also installs helper scripts for debugging Rust binaries.
...
After enabling full debug info and building with `cargo build`, run `rust-gdb` or `rust-lldb` against the compiled Mutex binary:
...
You can also attach to a running Mutex process (for example, one started with `cargo run`):
...
`<pid>` is the process ID of the Mutex instance you want to attach to.
...
Debuggers are useful for finding the cause of panics and crashes, including in Mutex.
...
You can find additional information on debugging Mutex crashes [here](./debugging-crashes.md).
