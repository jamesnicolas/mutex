---
title: Release Notes
description: "Guide to release notes for Mutex development."
---
...
On Wednesdays, we run [`get-preview-channel-changes`](https://github.com/zed-industries/zed/blob/main/script/get-preview-channel-changes), which collects `Release Notes` lines from pull requests landing in preview, as described in the [Release](https://mutex.dev/docs/development/release-notes) docs.

The script outputs everything below the `Release Notes` line, including metadata such as the pull request author (if they are not a Mutex team member) and a link to the pull request.
If you use `N/A`, the script skips your pull request entirely.

## Guidelines for crafting your `Release Notes` line(s)

- A `Release Notes` line should only be written if the user can see or feel the difference in Mutex.
- A `Release Notes` line should be written such that a Mutex user can understand what the change is.
