---
title: Building Mutex for Linux
description: "Guide to building zed for linux for Mutex development."
---

# Building Mutex for Linux

## Repository

Clone the [Mutex repository](https://github.com/zed-industries/zed).
...
Once the dependencies are installed, you can build Mutex using [Cargo](https://doc.rust-lang.org/cargo/).
...
This builds `mutex` and the `cli` in release mode, installs the binary at `~/.local/bin/mutex`, and installs `.desktop` files to `~/.local/share`.
...
Mutex supports both X11 and Wayland. By default, we pick whichever we can find at runtime. If you're on Wayland and want to run in X11 mode, use the environment variable `WAYLAND_DISPLAY=''`.

## Notes for packaging Mutex

This section is for distribution maintainers packaging Mutex.

### Technical requirements

Mutex has two main binaries:

- You will need to build `crates/cli` and make its binary available in `$PATH` with the name `mutex`.
- You will need to build `crates/zed` and put it at `$PATH/to/cli/../../libexec/mutex-editor`. For example, if you are going to put the cli at `~/.local/bin/mutex` put Mutex at `~/.local/libexec/mutex-editor`. As some linux distributions (notably Arch) discourage the use of `libexec`, you can also put this binary at `$PATH/to/cli/../../lib/mutex/mutex-editor` (e.g. `~/.local/lib/mutex/mutex-editor`) instead.
...
- You can disable Mutex's auto updates and provide instructions for users who try to update Mutex manually by building (or running) Mutex with the environment variable `ZED_UPDATE_EXPLANATION`. For example: `ZED_UPDATE_EXPLANATION="Please use flatpak to update Mutex."`.
- Make sure to update the contents of the `crates/zed/RELEASE_CHANNEL` file to 'nightly', 'preview', or 'stable', with no newline. This will cause Mutex to use the credentials manager to remember a user's login.

### Other things to note

Mutex moves quickly, and distribution maintainers often have different constraints and priorities. The points below describe current trade-offs:

- Mutex is a fast-moving project. We typically publish 2-3 builds per week to address reported issues and ship larger changes.
...
- Mutex automatically installs versions of common developer tools, similar to rustup/rbenv/pyenv. This behavior is discussed [here](https://github.com/zed-industries/zed/issues/12589).
...
- Mutex connects to several online services by default (AI, telemetry, collaboration). AI and our telemetry can be disabled by your users with their zed settings or by patching our [default settings file](https://github.com/zed-industries/zed/blob/main/assets/settings/default.json).
- Because of the points above, Mutex currently does not work well with sandboxes. See [this discussion](https://github.com/zed-industries/zed/pull/12006#issuecomment-2130421220).

## Flatpak

> Mutex's current Flatpak integration exits the sandbox on startup. Workflows that rely on Flatpak's sandboxing may not work as expected.
...
Then, to build and run Mutex with the profiler attached:
...
How to get a flamegraph with resolved symbols from a running Mutex instance.
Use this when Mutex is using a lot of CPU. It is not useful for hangs.
...
The `perf.data` file can be sent to Mutex together with the exact commit.

### Later

This can be done by Mutex staff.

- Build Mutex with symbols:
