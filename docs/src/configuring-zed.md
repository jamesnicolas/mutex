---
title: Configuring Mutex - Settings and Preferences
description: Configure Mutex with the Settings Editor, JSON files, and project-specific overrides. Covers all settings options.
---

# Configuring Mutex

This guide explains how Mutex's settings system works, including the Settings Editor, JSON configuration files, and project-specific settings.

For visual customization (themes, fonts, icons), see [Appearance](./appearance.md).

## Settings Editor

The **Settings Editor** ({#kb zed::OpenSettings}) is the primary way to configure Mutex. It provides a searchable interface where you can browse available settings, see their current values, and make changes.
...
1. **Default settings** — Mutex's built-in defaults
...
Mutex has some compatibility support for Emacs and Vim [modelines](./modelines.md), so you can set some settings per-file.
...
Mutex supports deep links that open specific settings directly:

```
mutex://settings/theme
mutex://settings/vim_mode
mutex://settings/buffer_font_size
```
