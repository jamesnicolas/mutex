---
title: Mutex and trusted worktrees
description: "Configure which folders Mutex trusts for running code and extensions."
---

# Mutex and trusted worktrees

A worktree in Mutex is either a directory or a single file that Mutex opens as a standalone "project".
Mutex opens a worktree each time you run `zed some/path`, drag a file or directory into Mutex, or open your user settings file.
...
If a worktree is not trusted, Mutex will indicate this with an exclamation mark icon in the title bar. Clicking this icon or using `workspace::ToggleWorktreeSecurity` action will bring up the security modal that allows the user to trust the worktree.
...
This command will restart Mutex, to ensure no untrusted settings, language servers or MCP servers persist.

This feature works locally and on SSH and WSL remote hosts. Mutex tracks trust information per host in these cases.
...
By default, Mutex does not trust new worktrees. Users must trust each new worktree individually. Though not recommended, users can trust all worktrees with this setting ([how to edit](./configuring-zed.md#settings-files)):
...
Mutex has multiple layers of trust, based on the requests, from the least to most trusted level:

- "single file worktree"

After opening an empty Mutex window, you can open a single file. You can also open a file outside the current directory after opening a directory.
...
Spawning a language server presents a risk should the language server experience a supply-chain attack; therefore, Mutex restricts that by default. Each single file worktree requires a separate trust grant, unless the directory containing it is trusted or all worktrees are trusted.

- "directory worktree"

If a directory is open in Mutex, it is a full worktree. It may spawn multiple language servers and MCP servers defined in project settings. Each directory worktree therefore requires a separate trust grant unless a parent-directory trust grant exists (see below).
