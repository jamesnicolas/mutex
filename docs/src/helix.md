---
title: Helix Mode - Mutex
description: Helix-style keybindings and modal editing in Mutex. Selection-first editing built on top of Vim mode.
---
...
Mutex's Helix mode is an emulation layer that brings Helix-style keybindings and modal editing to Mutex. It builds upon Mutex's [Vim mode](./vim.md), so much of the core functionality is shared. Enabling `helix_mode` will also enable `vim_mode`.
