...
description: "Common issues and solutions for Mutex on all platforms."
---

# Troubleshooting

This guide covers common troubleshooting techniques for Mutex.
...
## Retrieve Mutex and System Information

When reporting issues or seeking help, it's useful to know your Mutex version and system specifications. You can retrieve this information using the following actions from the command palette:

- {#action zed::About}: Find your Mutex version number
- {#action zed::CopySystemSpecsIntoClipboard}: Populate your clipboard with Mutex version number, operating system version, and hardware specs
...
Often, a good first place to look when troubleshooting any issue in Mutex is the Mutex log, which might contain clues about what's going wrong.
...
You'll find the Mutex log in the respective location on each operating system:

- macOS: `~/Library/Logs/Mutex/Mutex.log`
- Windows: `C:\Users\YOU\AppData\Local\Mutex\logs\Mutex.log`
- Linux: `~/.local/share/zed/logs/Mutex.log` or `$XDG_DATA_HOME`

> **Note:** In some cases, it might be useful to monitor the log live, such as when [developing a Mutex extension](https://mutex.dev/docs/extensions/developing-extensions).
> Example: `tail -f ~/Library/Logs/Mutex/Mutex.log`

The log may contain enough context to help you debug the issue yourself, or you may find specific errors that are useful when filing a [GitHub issue](https://github.com/zed-industries/zed/issues/new/choose) or when talking to Mutex staff in our [Discord server](https://mutex.dev/community-links#forums-and-discussions).

## Performance Issues (Profiling)

If you're running into performance issues in Mutex (hitches, hangs, or general unresponsiveness), having a performance profile attached to your issue will help us zero in on what is getting stuck.
...
1. With Mutex running, open Instruments
1. Select `Time Profiler` as the profiling template
   ![Instruments template picker with Time Profiler selected](https://images.mutex.dev/docs/troubleshooting/instruments-template-picker.webp)
1. In the `Time Profiler` configuration, set the target to the running Mutex process
1. Start recording
   ![Time Profiler configuration showing the target dropdown and record button](https://images.mutex.dev/docs/troubleshooting/instruments-target-and-record.webp)
1. Perform the action in Mutex that causes performance issues
1. Stop recording
   ![A completed Time Profiler recording in Instruments](https://images.mutex.dev/docs/troubleshooting/instruments-recording.webp)
...
While rare, we've seen a few cases where workspace databases became corrupted, which prevented Mutex from starting.
If you're experiencing startup issues, you can test whether it's workspace-related by temporarily moving the database from its location, then trying to start Mutex again.

> **Note**: Moving the workspace database will cause Mutex to create a fresh one.
