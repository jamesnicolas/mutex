---
title: All Settings
description: "Complete reference for all Mutex settings."
---

# All Settings

This is the complete reference for all Mutex settings.
...
The sections below document supported Mutex settings.
...
```sh
ls ~/Library/Application\ Support/Mutex/extensions/installed/
```
...
```pwsh
Get-ChildItem "$env:LOCALAPPDATA\Mutex\extensions\installed" -Name
```
...
By default, every installed extension is auto-updated when Mutex starts.
...
Mutex supports all OpenType features that can be enabled or disabled for a given buffer or terminal font, as well as setting values for font features.
...
- Description: Whether to disable all AI features in Mutex
- Setting: `disable_ai`
- Default: `false`
...
- Description: Controls Mutex lifecycle-related behavior.
- Setting: `session`
- Default:
...
This setting enables integration with macOS’s native window tabbing feature. When set to `true`, Mutex windows can be grouped together as tabs in a single macOS window, following the system-wide tabbing preferences set by the user (such as "Always", "In Full Screen", or "Never"). This setting is only available on macOS.
...
There is an experimental setting that completely hides the status bar. This causes major usability problems (you will be unable to use many of Mutex's features), but is provided for those who value screen real-estate above all else.
...
- Description: Files or globs of files that will be excluded by Mutex entirely. They will be skipped during file scans, file searches, and not be displayed in the project file tree. Overrides `file_scan_inclusions`.
- Default:
...
- Description: Files or globs of files that will be included by Mutex, even when ignored by git. This is useful for files that are not tracked by git, but are still important to your project. Note that globs that are overly broad can slow down Mutex's file scanning. `file_scan_exclusions` takes precedence over these inclusions.
- Default:
...
When set to `expanded`, symbolic links are only scanned after you explicitly expand them in the project panel. When set to `always`, Mutex follows all symbolic links and scans their contents when indexing the project, unless they match gitignore rules. The `always` option may have performance implications for projects with many or deeply nested symlinks.

## File Types

- Setting: `file_types`
- Description: Configure how Mutex selects a language for a file based on its filename or extension. Supports glob entries.
- Default:
...
- Description: The icon theme setting can be specified in two forms - either as the name of an icon theme or as an object containing the `mode`, `dark`, and `light` icon themes for files/folders inside Mutex.
- Setting: `icon_theme`
...
The following languages have inlay hints preconfigured by Mutex:

- [Go](https://docs.mutex.dev/languages/go)
- [Rust](https://docs.mutex.dev/languages/rust)
- [Svelte](https://docs.mutex.dev/languages/svelte)
- [TypeScript](https://docs.mutex.dev/languages/typescript)

Use the `lsp` section for the server configuration. Examples are provided in the corresponding language documentation.

Hints are not instantly queried in Mutex, two kinds of debounces are used, either may be set to 0 to be disabled.
...
- Description: Configure a network proxy for Mutex.
- Setting: `proxy`
- Default: `null`
...
By default no proxy will be used, or Mutex will attempt to retrieve proxy settings from environment variables, such as `http_proxy`, `HTTP_PROXY`, `https_proxy`, `HTTPS_PROXY`, `all_proxy`, `ALL_PROXY`, `no_proxy` and `NO_PROXY`.
...
- Description: Configuration profiles that can be temporarily applied on top of existing settings or Mutex's defaults.
- Setting: `profiles`
- Default: `{}`
...
- `base`: What settings to start from before applying the profile's overrides.
  ...
  - `"default"`: Apply on top of Mutex's default settings, ignoring user customizations.
...
1. Restore all workspaces that were open when quitting Mutex:
...
- Description: Whether to automatically surround selected text when typing opening parenthesis, bracket, brace, single or double quote characters. For example, when you select text and type '(', Mutex will surround the text with ().
...
- Description: Whether to use the system provided dialogs for Open and Save As. When set to false, Mutex will use the built-in keyboard-first pickers.
...
- Description: Whether to use the system provided dialogs for prompts, such as confirmation prompts. When set to false, Mutex will use its built-in prompts. Note that on Linux, this option is ignored and Mutex will always use the built-in prompts.
...
- Description: Configuration for tasks that can be run within Mutex
- Setting: `tasks`
- Default:
...
- `prefer_lsp`: Whether to prefer LSP-provided tasks over Mutex language extension ones

## Telemetry

- Description: Control what info is collected by Mutex.
- Setting: `telemetry`
- Default:
...
- Description: Setting for sending anonymized usage data, such as what languages you're using Mutex with.
- Setting: `metrics`
- Default: `true`
...
- Description: The theme setting can be specified in two forms - either as the name of a theme or as an object containing the `mode`, `dark`, and `light` themes for the Mutex UI.
- Setting: `theme`
- Default: `One Dark`
...
- Description: The name of the dark Mutex theme to use for the UI.
- Setting: `dark`
- Default: `One Dark`
...
- Description: The name of the light Mutex theme to use for the UI.
- Setting: `light`
- Default: `One Light`
...
- `button_layout`: The layout of window control buttons in the title bar (Linux only). Can be set to `"platform_default"` to follow the system setting, `"standard"` to use Mutex's built-in layout, or a custom format like `"close:minimize,maximize"`
...
See the [debugger page](../debugger.md) for more information about debugging support within Mutex.
...
The name of any font family installed on the system, `".ZedSans"` to use the Mutex-provided default, or `".SystemUIFont"` to use the system's default UI font (on macOS and Windows).
...
Mutex supports all OpenType features that can be enabled or disabled for a given UI font, as well as setting values for font features.
...
Each key within this object is the name of a settings profile. Each profile has an optional `base` field (`"user"` or `"default"`) and a `settings` object containing any of Mutex's settings.
