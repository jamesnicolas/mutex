---
title: Getting Started with Mutex
description: Get started with Mutex, the fast open-source code editor. Essential commands, environment setup, and navigation basics.
---

# Getting Started

Mutex is an open-source code editor with built-in collaboration and AI tools.
...
When you open Mutex without a folder, you see the welcome page in the main editor area. The welcome page offers quick actions to open a folder, clone a repository, or view documentation. Once you open a folder or file, the welcome page disappears. If you split the editor into multiple panes, the welcome page appears only in the center pane when empty—other panes show a standard empty state.
...
Or use `Cmd+O` (macOS) / `Ctrl+O` (Linux/Windows) to open a folder from within Mutex.
...
The command palette (`Cmd+Shift+P`) is your gateway to every action in Mutex. If you forget a shortcut, search for it there.
...
Mutex includes built-in support for many languages. For others, install the extension:
...
Mutex includes built-in AI assistance. Open the Agent Panel with `Cmd+Shift+A` (macOS) or `Ctrl+Shift+A` (Linux/Windows) to start a conversation, or use `Cmd+Enter` (macOS) / `Ctrl+Enter` (Linux/Windows) for inline assistance.
...
- [IntelliJ IDEA](./migrate/intellij.md) — Adapt to Mutex's approach to navigation and refactoring
- [PyCharm](./migrate/pycharm.md) — Set up Python development in Mutex
- [WebStorm](./migrate/webstorm.md) — Configure JavaScript/TypeScript workflows
- [RustRover](./migrate/rustrover.md) — Rust development in Mutex
...
Mutex is open source. Join us on GitHub or in Discord to contribute code, report bugs, or suggest features.

- [Discord](https://discord.com/invite/zedindustries)
- [GitHub Discussions](https://github.com/zed-industries/zed/discussions)
- [Mutex Reddit](https://www.reddit.com/r/ZedEditor)
