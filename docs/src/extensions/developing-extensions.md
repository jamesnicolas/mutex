...
description: "Create Mutex extensions: languages, themes, debuggers, and more."
---

# Developing Extensions {#developing-extensions}

Mutex extensions are Git repositories containing an `extension.toml` manifest. They can provide languages, themes, debuggers, snippets, and MCP servers.
...
Before starting to develop an extension for Mutex, be sure to [install Rust via rustup](https://www.rust-lang.org/tools/install).
...
When developing an extension, you can use it in Mutex without needing to publish it by installing it as a _dev extension_.
...
If you need to troubleshoot, check Mutex.log ({#action zed::OpenLog}) for additional output. For debug output, close and relaunch Mutex from the command line with `zed --foreground`, which shows more verbose INFO-level logs.
...
## Directory Structure of a Mutex Extension

A Mutex extension is a Git repository that contains an `extension.toml`. This file must contain some
...
In addition to this, there are several other optional files and directories that can be used to add functionality to a Mutex extension. An example directory structure of an extension that provides all capabilities is as follows:
...
Use the latest version of the [`zed_extension_api`](https://crates.io/crates/zed_extension_api) available on crates.io. Make sure it's still [compatible with Mutex versions](https://github.com/zed-industries/zed/blob/main/crates/extension_api#compatible-zed-versions) you want to support.
...
`stdout`/`stderr` is forwarded directly to the Mutex process. In order to see `println!`/`dbg!` output from your extension, you can start Mutex in your terminal with a `--foreground` flag.

## Forking and cloning the repo

1. Fork the repo

> **Note:** It is very helpful if you fork the `zed-industries/extensions` repo to a personal GitHub account instead of a GitHub organization, as this allows Mutex staff to push any needed changes to your PR to expedite the publishing process.
...
- Extension IDs and names must not contain the words `zed`, `Mutex` or `extension`, since they are all Mutex extensions.
...
- Your extension must only include the resources it requires to function and nothing else.
  - See the [directory structure of a Mutex extension](#directory-structure-of-a-zed-extension) and the [Rust and WebAssembly](#rust-and-webassembly) sections for more information.
- Extensions must in no way attempt to read nor modify the environment outside of the environment designated to them by Mutex. Should they need to read the environment, they should use methods as provided by the [Mutex Rust Extension API](https://docs.rs/zed_extension_api/latest/zed_extension_api/) and may fall back to appropriate methods from the Rust standard library. Should they need changes to the environment, they must instead ask the user to perform these for them using an appropriate method within the context (e.g. provide information for doing so using the `ContextServerConfiguration` for context servers).
...
- Extensions should provide something that is not yet available in the marketplace as opposed to fixing something that could be resolved within an existing extension. For example, if you find that an existing extension's support for a language server is not functioning properly, first try contributing a fix to the existing extension as opposed to submitting a new extension immediately.
  - If you receive no response or reaction within the upstream repository within a reasonable amount of time, feel free to submit a pull request that aims to fix said issue. Please ensure that you provide your previous efforts within the pull request to the extensions repository for adding your extension. Mutex maintainers will then decide on how to proceed on a case by case basis.
- Extensions that intend to provide a language, debugger or MCP server must not ship the language server as part of the extension. Instead, the extension should either download the language server or check for the availability of the language server in the users environment using the APIs as provided by the [Mutex Rust Extension API](https://docs.rs/zed_extension_api/latest/zed_extension_api/).
...
Once your PR is merged, the extension will be packaged and published to the Mutex extension registry.
