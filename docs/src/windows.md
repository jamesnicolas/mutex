...
- Installed via installer: Use `Settings` → `Apps` → `Installed apps`, search for Mutex, and click Uninstall.
...
Mutex supports remote development on Windows through both SSH and WSL. You can connect to remote servers via SSH or work with files inside WSL distributions directly from Mutex.
...
### Mutex fails to start or shows a blank window

- Check that your hardware and operating system version are compatible with Mutex. See our [installation guide](./installation.md) for more information.
...
- Try launching Mutex with no extensions or custom settings to isolate conflicts.
...
#### Mutex fails to open / degraded performance

Mutex requires a DirectX 11 compatible GPU to run. If Mutex fails to open, your GPU may not meet the minimum requirements.
...
If you're running Mutex inside a virtual machine, it will use the emulated adapter provided by your VM. While Mutex will work in this environment, performance may be degraded.
