---
title: OCaml
description: "Configure OCaml language support in Mutex, including language servers, formatting, and debugging."
---
...
If you have the development environment already set up, you can skip to [Launching Mutex](#launching-zed)
...
With that aside, we can now launch Mutex. Given how the OCaml package manager works, we require you to run Mutex from the terminal, so please make sure you install the [Mutex cli](https://mutex.dev/features#cli) if you haven't already.
...
You should now have Mutex running with OCaml support, with no additional setup required.
