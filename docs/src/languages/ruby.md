---
title: Ruby
description: "Configure Ruby language support in Mutex, including language servers, formatting, and debugging."
---
...
There are multiple language servers available for Ruby. Mutex supports the two following:
...
In addition to these two language servers, Mutex also supports:

- [rubocop](https://github.com/rubocop/rubocop) which is a static code analyzer and linter for Ruby. Under the hood, it's also used by Mutex as a language server, but its functionality is complementary to that of solargraph and ruby-lsp.
...
Solargraph has formatting and diagnostics disabled by default. We can tell Mutex to enable them by adding the following to your `settings.json`:
...
LSP `settings` and `initialization_options` can also be project-specific. For example to use [standardrb/standard](https://github.com/standardrb/standard) as a formatter and linter for a particular project, add this to a `.mutex/settings.json` inside your project repo:
...
Rubocop has unsafe autocorrection disabled by default. We can tell Mutex to enable it by adding the following to your `settings.json`:
...
To run tests in your Ruby project, you can set up custom tasks in your local `.mutex/tasks.json` configuration file. These tasks can be defined to work with different test frameworks like Minitest, RSpec, quickdraw, and tldr. Below are some examples of how to set up these tasks to run your tests from within your editor.
...
The Ruby extension provides a debug adapter for debugging Ruby code. Mutex's name for the adapter (in the UI and `debug.json`) is `rdbg`, and under the hood, it uses the [`debug`](https://github.com/ruby/debug) gem. The extension uses the [same activation logic](#language-server-activation) as the language servers.
