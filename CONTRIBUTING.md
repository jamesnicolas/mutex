...
Thank you for helping us make Mutex better!

All activity in Mutex forums is subject to our [Code of
Conduct](https://mutex.dev/code-of-conduct). Additionally, contributors must sign
our [Contributor License Agreement](https://mutex.dev/cla) before their
contributions can be merged.

## Contribution ideas

Mutex is a large project with a number of priorities. We spend most of
...
- Part of a **Community Program** like [Let's Git Together](https://github.com/zed-industries/zed/issues/41541) or [The Guild](https://mutex.dev/community/guild).
...
- Make sure the change is **desired**: we're always happy to accept bugfixes,
  ...
  - This especially applies to any changes proposed to the Mutex Extension API.
- Include a clear description of **what you're solving**, and why it's important.
- Include **tests**. For UI changes, consider updating visual regression tests (see [Building Mutex for macOS](./docs/src/development/macos.md#visual-regression-tests)).
...
- Anything that can be provided by an extension. For example a new language, or theme. For adding themes or support for a new language to Mutex, check out our [docs on developing extensions](https://mutex.dev/docs/extensions/developing-extensions).
- Changes to the Mutex Extension API submitted without prior discussion involving Mutex staff.
- New file icons. Mutex's default icon theme consists of icons that are hand-designed to fit together in a cohesive manner, please don't submit PRs with off-the-shelf SVGs.
...
- Does it match Mutex’s design language (spacing, typography, icons)?
  - Make sure to visit [the icon design guidelines](https://github.com/zed-industries/zed/blob/main/crates/icons/README.md)
- Are terminology, labels, and tone consistent with the rest of Mutex?
...
- Do we avoid internal Mutex jargon that only insiders would know?
...
- Are error messages actionable and consistent with Mutex’s voice?
...
## Bird's-eye view of Mutex

We suggest you keep the [Mutex glossary](docs/src/development/glossary.md) at your side when starting out. It lists and explains some of the structures and terms you will see throughout the codebase.

Mutex is made up of several smaller crates - let's go over those you're most likely to interact with:

- [`gpui`](/crates/gpui) is a GPU-accelerated UI framework which provides all of the building blocks for Mutex. **We recommend familiarizing yourself with the root level GPUI documentation.**
- [`editor`](/crates/editor) contains the core `Editor` type that drives both the code editor and all various input fields within Mutex. It also handles a display layer for LSP features such as Inlay Hints or code completions.
- [`project`](/crates/project) manages files and navigation within the filetree. It is also Mutex's side of communication with LSP.
...
- [`ui`](/crates/ui) is a collection of UI components and common patterns used throughout Mutex.
- [`cli`](/crates/cli) is the CLI crate which invokes the Mutex binary.
- [`zed`](/crates/zed) is where all things come together, and the `main` entry point for Mutex.

## Packaging Mutex

Check our [notes for packaging Mutex](https://mutex.dev/docs/development/linux#notes-for-packaging-zed).
