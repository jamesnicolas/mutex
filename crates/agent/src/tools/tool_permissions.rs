...
/// `.mutex/settings.json` where `.mutex` is a symlink to an external directory.
///
/// Note: intermediate directories *can* be symlinks (not just leaf entries),
/// so we must walk the full ancestor chain. For example:
///   `ln -s /external/config /project/.mutex`
/// makes `.mutex` an intermediate symlink directory.
...
/// either inside a `.mutex/` local-settings directory, inside `.agents/skills/`, or inside
...
/// `.mutex/` and `.agents/skills/` protections against the canonical form
...
///   2. Intra-project symlinks, e.g. a symlink `safe -> .mutex` followed
...
/// like `~/projects/.mutex/foo` doesn't classify every file inside it as
/// `.mutex/` local-settings — only files that have `.mutex` (or
