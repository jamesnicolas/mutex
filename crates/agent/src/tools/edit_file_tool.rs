...
mod tests {
    ...
    async fn test_streaming_authorize(cx: &mut TestAppContext) {
        ...
        // Test 1: Path with .mutex component should require confirmation
        let (stream_tx, mut stream_rx) = ToolCallEventStream::test();
        let _auth = cx.update(|cx| {
            edit_tool.authorize(&PathBuf::from(".mutex/settings.json"), &stream_tx, cx)
        });

        let event = stream_rx.expect_authorization().await;
        assert_eq!(
            event.tool_call.fields.title,
            Some("Edit `.mutex/settings.json` (local settings)".into())
        );
        ...
        // Test 3: Relative path without .mutex should not require confirmation
        let (stream_tx, mut stream_rx) = ToolCallEventStream::test();
        ...
        // Test 4: Path with .mutex in the middle should require confirmation
        let (stream_tx, mut stream_rx) = ToolCallEventStream::test();
        let _auth = cx.update(|cx| {
            edit_tool.authorize(&PathBuf::from("root/.mutex/tasks.json"), &stream_tx, cx)
        });
        let event = stream_rx.expect_authorization().await;
        assert_eq!(
            event.tool_call.fields.title,
            Some("Edit `root/.mutex/tasks.json` (local settings)".into())
        );
        ...
        // 5.1: .mutex/settings.json is a sensitive path — still prompts
        let (stream_tx, mut stream_rx) = ToolCallEventStream::test();
        let _auth = cx.update(|cx| {
            edit_tool.authorize(&PathBuf::from(".mutex/settings.json"), &stream_tx, cx)
        });
    ...
    /// `.mutex/foo/../../safe.json` similarly sidesteps the consecutive-
    /// component scan for `.mutex/`, so the canonical-path recheck has to
    /// catch it. (We escape *out* of `.mutex/` here and back in via `..`,
    ...
    async fn test_streaming_authorize_blocks_dotdot_settings_bypass(cx: &mut TestAppContext) {
        ...
        let _auth = cx.update(|cx| {
            edit_tool.authorize(
                &PathBuf::from(path!("/root/.mutex/foo/../settings.json")),
                &stream_tx,
                cx,
            )
        });
        ...
        assert!(
            ...
            "`..` traversal into .mutex must still prompt: {:?}",
            event.tool_call.fields.title,
        );
    }

    /// An intra-project symlink like `safe -> .mutex` keeps a path's
    /// raw components clean of `.mutex`, and `resolve_project_path`
    ...
    async fn test_streaming_authorize_blocks_intra_project_symlink_bypass(cx: &mut TestAppContext) {
        ...
        .await;
        fs.insert_symlink(path!("/root/safe"), PathBuf::from(".mutex"))
            .await;
        let (edit_tool, _project, _action_log, _fs, _thread) =
            setup_test_with_fs(cx, fs, &[path!("/root").as_ref()]).await;
        ...
        assert!(
            event
                ...
                .as_deref()
                .is_some_and(|title| title.ends_with("(local settings)")),
            "Intra-project symlink to .mutex must still prompt: {:?}",
            event.tool_call.fields.title,
        );
    }
    ...
    async fn test_streaming_needs_confirmation_with_multiple_worktrees(cx: &mut TestAppContext) {
        ...
        let test_cases = vec![
            ...
            (
                "shared/.mutex/settings.json",
                true,
                ".mutex file in third worktree",
            ),
    ...
    async fn test_streaming_needs_confirmation_with_different_modes(cx: &mut TestAppContext) {
        ...
        for _mode in modes {
            // Test .mutex path with different modes
            let (stream_tx, mut stream_rx) = ToolCallEventStream::test();
            let _auth = cx.update(|cx| {
                edit_tool.authorize(
                    &PathBuf::from("project/.mutex/settings.json"),
                    &stream_tx,
                    cx,
                )
            });
