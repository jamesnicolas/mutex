...
pub fn compile(manifest: bool) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (icon_filename, product_name) = match channel {
        "stable" => ("app-icon.ico", "Mutex"),
        "preview" => ("app-icon-preview.ico", "Mutex Preview"),
        "nightly" => ("app-icon-nightly.ico", "Mutex Nightly"),
        _ => ("app-icon-dev.ico", "Mutex Dev"),
    };
...
BEGIN
    ...
    BEGIN
        ...
        BEGIN
            ...
            VALUE "CompanyName", "Mutex Industries, Inc.\0"
            VALUE "LegalCopyright", "Copyright 2022 - 2025 Mutex Industries, Inc.\0"
        END
    END
    BLOCK "VarFileInfo"
    BEGIN
