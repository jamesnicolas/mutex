...
//! pipeline. Mutex opts into that pipeline during rendering, then keeps
...
//! - [`postprocess`] the SVG to add Mutex theme and accent styling.
//!
//! Mutex's postprocessing is split up into stages. We parse the generated SVG
...
//! `player_colors` in the Mutex theme.
//!
...
/// from Mutex's UI palette) round-trip without silently losing their alpha.
