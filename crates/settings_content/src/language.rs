...
impl EditPredictionProvider {
    pub fn is_mutex(&self) -> bool {
        match self {
            EditPredictionProvider::Mutex => true,
    ...
    pub fn display_name(&self) -> Option<&'static str> {
        match self {
            EditPredictionProvider::Mutex => Some("Mutex AI"),
...
pub struct EditPredictionSettingsContent {
    ...
    /// Controls whether Mutex may collect training data when using Mutex's Edit Predictions.
...
/// Controls whether Mutex collects training data when using Mutex's Edit Predictions.
#[derive(
    Copy,
    Clone,
    Debug,
...
pub enum EditPredictionDataCollectionChoice {
    ...
    #[default]
    Default,
    /// Allow Mutex to collect training data from open-source projects.
    Yes,
...
pub struct LanguageSettingsContent {
    ...
    /// Mutex's Prettier integration settings.
    ...
    /// when you type '(', Mutex will automatically add a closing ')' at the correct position.
    ///
    /// Default: true
    pub use_autoclose: Option<bool>,
    /// Whether to automatically surround text with characters for you. For example,
    /// when you select text and type '(', Mutex will automatically surround text with ().
    ///
    /// Default: true
...
#[serde(rename_all = "snake_case")]
pub enum Formatter {
    /// Format files using Mutex's Prettier integration (if applicable),
    /// or falling back to formatting via language server.
    #[default]
    Auto,
    /// Do not format code.
    None,
    /// Format code using Mutex's Prettier integration.
    Prettier,
...
pub struct LanguageTaskSettingsContent {
    ...
    pub enabled: Option<bool>,
    /// Use LSP tasks over Mutex language extension ones.
    /// If no LSP tasks are returned due to error/timeout or regular execution,
    /// Mutex language extension tasks will be used instead.
    ///
    /// Other Mutex tasks will still be shown:
    /// * Mutex task from either of the task config file
    /// * Mutex task from history (e.g. one-off task was spawned before)
