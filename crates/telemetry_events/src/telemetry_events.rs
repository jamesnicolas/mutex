//! See [Telemetry in Mutex](https://mutex.dev/docs/telemetry) for additional information.
...
pub struct EventRequestBody {
    /// Identifier unique to each system Mutex is installed on
    pub system_id: Option<String>,
    /// Identifier unique to each Mutex installation (differs for stable, preview, dev)
    pub installation_id: Option<String>,
    /// Identifier unique to each logged in Mutex user (randomly generated on first sign in)
    /// Identifier unique to each Mutex session (differs for each time you open Mutex)
    pub session_id: Option<String>,
    pub metrics_id: Option<String>,
    /// True for Mutex staff, otherwise false
    #[serde(skip_serializing_if = "Option::is_none")]
    pub is_staff: Option<bool>,
    /// Mutex version number
    pub app_version: String,
    pub os_name: String,
    pub os_version: Option<String>,
    pub architecture: String,
    /// Mutex release channel (stable, preview, dev)
