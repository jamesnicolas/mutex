//! Paths to locations used by Mutex.
...
///
/// Forks should change this to avoid colliding with Mutex's user data.
pub const APP_NAME: &str = "Mutex";
...
/// On macOS, this is `~/Library/Application Support/Mutex`.
/// On Linux/FreeBSD, this is `$XDG_DATA_HOME/mutex`.
/// On Windows, this is `%LOCALAPPDATA%\Mutex`.
static CURRENT_DATA_DIR: OnceLock<PathBuf> = OnceLock::new();

/// The resolved config directory, combining custom override or platform defaults.
/// This is set once and cached for subsequent calls.
/// On macOS, this is `~/.config/mutex`.
/// On Linux/FreeBSD, this is `$XDG_CONFIG_HOME/mutex`.
/// On Windows, this is `%APPDATA%\Mutex`.
static CONFIG_DIR: OnceLock<PathBuf> = OnceLock::new();

/// Returns the relative path to the mutex_server directory on the ssh host.
pub fn remote_server_dir_relative() -> &'static RelPath {
    static CACHED: LazyLock<&'static RelPath> =
        LazyLock::new(|| RelPath::unix(".mutex_server").unwrap());
    *CACHED
}

// Remove this once 223 goes stable
/// Returns the relative path to the mutex_wsl_server directory on the wsl host.
pub fn remote_wsl_server_dir_relative() -> &'static RelPath {
    static CACHED: LazyLock<&'static RelPath> =
        LazyLock::new(|| RelPath::unix(".mutex_wsl_server").unwrap());
    *CACHED
}
...
/// Returns the path to the configuration directory used by Mutex.
...
/// Returns the path to the data directory used by Mutex.
...
/// Returns the path to the temp directory used by Mutex.
...
/// Returns the path to the Mutex server directory on this SSH host.
...
/// Returns the path to the `Mutex.log` file.
pub fn log_file() -> &'static PathBuf {
    static LOG_FILE: OnceLock<PathBuf> = OnceLock::new();
    LOG_FILE.get_or_init(|| logs_dir().join(format!("{}.log", APP_NAME)))
}

/// Returns the path to the `Mutex.log.old` file.
...
/// user opens, and is loaded into the native Mutex agent's system prompt.
...
/// # Arguments
///
/// * `dev_mode` - If true, assumes the current working directory is the Mutex repository.
...
///
/// This is where language servers are downloaded to for languages built-in to Mutex.
...
///
/// This is where debug adapters are downloaded to for DAPs that are built-in to Mutex.
...
/// Returns the relative path to a `.mutex` folder within a project.
...
pub fn local_settings_file_relative_path() -> &'static RelPath {
    static CACHED: LazyLock<&'static RelPath> =
        LazyLock::new(|| RelPath::unix(".mutex/settings.json").unwrap());
    *CACHED
}
...
pub fn local_tasks_file_relative_path() -> &'static RelPath {
    static CACHED: LazyLock<&'static RelPath> =
        LazyLock::new(|| RelPath::unix(".mutex/tasks.json").unwrap());
    *CACHED
}
...
pub fn local_debug_file_relative_path() -> &'static RelPath {
    static CACHED: LazyLock<&'static RelPath> =
        LazyLock::new(|| RelPath::unix(".mutex/debug.json").unwrap());
    *CACHED
}
