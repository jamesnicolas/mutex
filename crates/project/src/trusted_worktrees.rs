//! A module, responsible for managing the trust logic in Mutex.
//!
...
//!
//! Mutex does not consider invisible, `worktree.is_visible() == false` worktrees in Mutex, as those are programmatically created inside Mutex for internal needs, e.g. a tmp dir for `keymap_editor.rs` needs.
//!
//!
//! Path rust hierarchy.
//!
//! Mutex has multiple layers of trust, based on the requests and [`PathTrust`] enum variants.
//! From the least to the most trusted level:
//!
//! * "single file worktree"
//!
//! After opening an empty Mutex it's possible to open just a file, same as after opening a directory in Mutex it's possible to open a file outside of this directory.
//! Usual scenario for both cases is opening Mutex's settings.json file via `zed: open settings file` command: that starts a language server for a new file open, which originates from a newly created, single file worktree.
//!
//! Spawning a language server is potentially dangerous, and Mutex needs to restrict that by default.
//! Each single file worktree requires a separate trust permission, unless a more global level is trusted.
//!
//! * "directory worktree"
//!
//! If a directory is open in Mutex, it's a full worktree which may spawn multiple language servers associated with it.
...
//! * "path override"
//!
//! To ease trusting multiple directory worktrees at once, it's possible to trust a parent directory of a certain directory worktree opened in Mutex.
...
impl TrustedWorktreesStore {
    ...
    /// Requires Mutex's restart to take proper effect.
    ...
    pub fn can_trust(
        ...
        // Mutex opened an "internal" directory: e.g. a tmp dir for `keymap_editor.rs` needs.
