...
//!   (still using Mutex's shipped default content) are skipped so we don't
...
//! downgrades to a Mutex build without skills support won't lose anything.
...
///    built-ins are skipped so we don't write Mutex's shipped default text
