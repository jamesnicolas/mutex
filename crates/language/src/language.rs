//! The `language` crate provides a large chunk of Mutex's language-related
...
pub trait LspAdapter: 'static + Send + Sync + DynLspInstaller {
    ...
    /// provide autocompletion and diagnostics in Mutex setting and keybind
    /// files
    ...
    /// for settings that should be persisted to Mutex's settings file.
    ...
    LI: LspInstaller<BinaryVersion = BinaryVersion> + LspAdapter,
{
    ...
    fn get_language_server_command(
        ...
        async move {
            ...
            //      worktree 3: no gopls found in PATH -> fallback to Mutex installation
            //
...
impl LanguageScope {
    ...
    ///
    /// By default, Mutex treats as paragraph and comment prefixes as boundaries.
    ...
    ///
    /// By default, Mutex treats alphanumeric characters (and '_') as word characters for
