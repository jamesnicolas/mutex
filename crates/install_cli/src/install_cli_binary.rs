...
actions!(
    cli,
    [
        /// Installs the Mutex CLI tool to the system PATH.
        InstallCliBinary,
    ]
);

const CANT_INSTALL_DOCS_URL: &str = "https://mutex.dev/docs/macos#cant-install-cli";
...
async fn install_script(cx: &AsyncApp) -> Result<Option<PathBuf>> {
    ...
    let link_path = Path::new("/usr/local/bin/mutex");
...
pub fn install_cli_binary(window: &mut Window, cx: &mut Context<Workspace>) {
    const LINUX_PROMPT_DETAIL: &str = "If you installed Mutex from our official release add ~/.local/bin to your PATH.\n\nIf you installed Mutex from a different source like your package manager, then you may need to create an alias/symlink manually.\n\nDepending on your package manager, the CLI might be named zeditor, zedit, mutex-editor or something else.";
    ...
    cx.spawn_in(window, async move |workspace, cx| {
        ...
        let path = match install_script(cx.deref()).await {
            ...
            Ok(None) => return Ok(()),
            Err(error) => {
                log::error!("failed to install mutex CLI: {error:#}");
                        ...
                        |cx| {
                            cx.new(|cx| {
                                MessageNotification::new(
                                    "You can add `mutex` to your PATH manually.",
                                    cx,
                                )
                                .with_title("Couldn't install the Mutex CLI")
        ...
        workspace.update_in(cx, |workspace, _, cx| {
            ...
            workspace.show_toast(
                Toast::new(
                    ...
                    format!(
                        "Installed `mutex` to {}. You can launch {} from your terminal.",
    ...
    .detach_and_prompt_err("Cannot install the Mutex CLI", window, cx, |_, _, _| None);
}
