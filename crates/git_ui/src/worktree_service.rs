...
/// one of its linked worktrees open as separate Mutex worktrees). Those entries
...
pub struct CreatedWorktreeWorkspace {
    ...
    /// True when the project contained more than one Mutex worktree backed by
...
async fn do_create_worktree(
    ...
    // Mutex created it before deleting it from disk. Failures are non-fatal:
