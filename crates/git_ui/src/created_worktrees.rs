//! A persistent registry of git worktrees that Mutex itself created.
//!
//! Thread archival deletes a worktree's directory from disk, so it must be
//! certain the worktree was created by Mutex rather than by the user. This
//! module records each Mutex-created worktree in the local database, keyed by
//! its path (and remote host, for remote projects), along with the creation
//! time of the worktree's git metadata directory at the time Mutex created it.
//!
...
//! recreated outside Mutex, so deletion must be skipped. Every failure mode
...
//! different Mutex install (e.g. another release channel, or another machine
...
/// Records that Mutex created the worktree at `worktree_path`, along with the
...
/// Returns the recorded creation time for a worktree Mutex created, or `None`
/// if Mutex has no record of creating it.
...
/// Removes the record for a worktree, either because Mutex deleted it or
/// because the directory on disk turned out not to be the one Mutex created.
