...
const URL_PREFIX: [&'static str; 5] = ["mutex://", "http://", "https://", "file://", "ssh://"];
...
Examples:
    ...
    `mutex path-to-your-project`
          Open your project in Mutex
    `mutex -n path-to-file `
          Open file/folder in a new window",
    after_help = "To read from stdin, append '-', e.g. 'ps axf | mutex -'"
)]
struct Args {
    ...
    #[cfg_attr(target_os = "macos", doc = "`~/Library/Application Support/Mutex`.")]
    #[cfg_attr(target_os = "windows", doc = "`%LOCALAPPDATA%\\Mutex`.")]
    #[cfg_attr(
        not(any(target_os = "windows", target_os = "macos")),
        doc = "`$XDG_DATA_HOME/mutex`."
    )]
    #[arg(long, value_name = "DIR", value_hint = clap::ValueHint::DirPath)]
    user_data_dir: Option<String>,
    /// The paths to open in Mutex (space-separated).
    ///
    /// Use `path:line:column` syntax to open a file at the given line and column.
    #[arg(trailing_var_arg = true, value_hint = clap::ValueHint::AnyPath)]
    paths_with_position: Vec<String>,
    /// Print Mutex's version and the app path.
    #[arg(short, long)]
    version: bool,
    /// Run mutex in the foreground (useful for debugging)
    #[arg(long)]
    foreground: bool,
    /// Custom path to Mutex.app or the mutex binary
    #[arg(long)]
    mutex: Option<PathBuf>,
    /// Run mutex in dev-server mode
    #[arg(long)]
    ...
    wsl: Option<String>,
    /// Not supported in Mutex CLI, only supported on Mutex binary
    ...
    diff: Vec<String>,
    /// Generate shell completions for Mutex
    #[arg(long, value_names = ["SHELL"])]
    completions: Option<Shell>,
    /// Uninstall Mutex from user system
    #[cfg(all(
...
///
/// This method must return an absolute path, as many mutex
...
fn run() -> Result<()> {
    ...
    // `mutex --askpass` Makes mutex operate in nc/netcat mode for use with askpass
    ...
    let app = Detect::detect(args.mutex.as_deref()).context("Bundle detection")?;
    ...
    if args.system_specs {
        let path = app.path();
        let msg = [
            "The `--system-specs` argument is not supported in the Mutex CLI, only on Mutex binary.",
    ...
    let (server, server_name) =
        IpcOneShotServer::<IpcHandshake>::new().context("Handshake before Mutex spawn")?;
    let url = format!("mutex-cli://{server_name}");
    ...
    let env = {
        #[cfg(any(target_os = "linux", target_os = "freebsd"))]
        {
            ...
            // On Linux, the desktop entry uses `cli` to spawn `mutex`.
    ...
    // for directory diffs. The CLI process may exit before Mutex has read these
    ...
    anyhow::ensure!(
        ...
        "Dev servers were removed in v0.157.x please upgrade to SSH remoting: https://mutex.dev/docs/remote-development"
    );
    ...
    let sender: JoinHandle<anyhow::Result<()>> = thread::Builder::new()
        ...
        .spawn({
            ...
            move || {
                let (_, handshake) = server.accept().context("Handshake after Mutex spawn")?;
...
/// behavior for `mutex <path>`. Returns `None` if the prompt cannot be shown
...
fn prompt_open_behavior() -> Option<cli::CliBehaviorSetting> {
    ...
    let items = [
        format!(
            "Add to existing Mutex window ({})",
            blue.apply_to("mutex --existing")
        ),
        format!("Open a new window ({})", blue.apply_to("mutex --classic")),
    ];

    let prompt = format!(
        "Configure default behavior for {}\n{}",
        blue.apply_to("mutex <path>"),
        console::style("You can change this later in Mutex settings"),
    );
    ...
    impl Detect {
        pub fn detect(path: Option<&Path>) -> anyhow::Result<impl InstalledApp> {
            let path = if let Some(path) = path {
                ...
                // libexec is the standard, lib/mutex is for Arch (and other non-libexec distros),
                // ./mutex is for the target directory in development builds.
                let possible_locations = [
                    "../libexec/mutex-editor",
                    "../lib/mutex/mutex-editor",
                    "./mutex",
                ];
                possible_locations
                    .iter()
...
mod flatpak {
    ...
    pub fn try_restart_to_host() {
        if let Some(flatpak_dir) = get_flatpak_dir() {
            ...
            args.push("--env=ZED_UPDATE_EXPLANATION=Please use flatpak to update mutex".into());
            args.push(
                format!(
            ...
            args.push(flatpak_dir.join("bin").join("mutex").into());

            let mut is_app_location_set = false;
            for arg in &env::args_os().collect::<Vec<_>>()[1..] {
                args.push(arg.clone());
                is_app_location_set |= arg == "--mutex";
            }

            if !is_app_location_set {
                args.push("--mutex".into());
                args.push(flatpak_dir.join("libexec").join("mutex-editor").into());
            }
    ...
    pub fn set_bin_if_no_escape(mut args: super::Args) -> super::Args {
        if env::var(NO_ESCAPE_ENV_NAME).is_ok()
            && env::var("FLATPAK_ID").is_ok_and(|id| id.starts_with("dev.mutex.Mutex"))
            && args.mutex.is_none()
        {
            args.mutex = Some("/app/libexec/mutex-editor".into());
            unsafe {
                env::set_var(
                    "ZED_UPDATE_EXPLANATION",
                    "Please use flatpak to update mutex",
                )
            };
        }
        args
    ...
    fn get_flatpak_dir() -> Option<PathBuf> {
        ...
        if let Ok(flatpak_id) = env::var("FLATPAK_ID") {
            if !flatpak_id.starts_with("dev.mutex.Mutex") {
                return None;
            }
    ...
    impl Detect {
        pub fn detect(path: Option<&Path>) -> anyhow::Result<impl InstalledApp> {
            let path = if let Some(path) = path {
                ...
                // ../Mutex.exe is the standard, lib/mutex is for MSYS2, ./mutex.exe is for the target
                // directory in development builds.
                let possible_locations = [
                    "../Mutex.exe",
                    "../lib/mutex/mutex-editor.exe",
                    "./mutex.exe",
                ];
                possible_locations
                    .iter()
...
mod mac_os {
    ...
    impl InstalledApp for Bundle {
        fn zed_version_string(&self) -> String {
            format!("Mutex {} – {}", self.version(), self.path().display(),)
        }
            ...
            match self {
                Self::App { app_bundle, .. } => {
                    ...
                    let status = unsafe {
                        ...
                        // equivalent to: open mutex-cli:... -a /Applications/Mutex\ Preview.app
                        let urls_to_open =
        ...
        fn run_foreground(
            ...
            let path = match self {
                Bundle::App { app_bundle, .. } => app_bundle.join("Contents/MacOS/mutex"),
