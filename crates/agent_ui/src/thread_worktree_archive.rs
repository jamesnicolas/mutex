...
pub struct RootPlan {
    ...
    /// recorded when Mutex created the worktree. [`remove_root`] re-stats the
    /// directory and refuses to delete anything if the time has changed,
    /// which means the worktree was recreated outside Mutex.
...
/// Resolves the Mutex-managed worktrees base directory for a given repo.
///
/// This intentionally reads the *global* `git.worktree_directory` setting
/// rather than any project-local override, because Mutex always uses the
...
pub fn build_root_plan(
    ...
    // Only archive worktrees that live inside the Mutex-managed worktrees
    ...
    // Only archive worktrees that Mutex explicitly created. The directory
    // check above constrains paths, but the database record is what
    // distinguishes a Mutex-created worktree from one the user manually
...
pub async fn remove_root(root: RootPlan, cx: &mut AsyncApp) -> Result<()> {
    ...
    // user later creates a new worktree at the same path outside Mutex, a
...
/// Confirms that the worktree on disk is still the one Mutex created, by
/// comparing the creation time of its git metadata directory against the
/// time recorded when Mutex created it.
///
/// Outcomes:
...
///   Mutex. The registry record is removed (so subsequent archival attempts
...
async fn verify_created_by_zed(root: &RootPlan, cx: &mut AsyncApp) -> Result<()> {
    ...
    let created_at = receiver
        ...
        .with_context(|| {
            format!(
                "refusing to delete worktree at {}: failed to verify that Mutex created it",
    ...
    match created_at {
        ...
        Some(_) => {
            ...
            .log_err();
            Err(anyhow!(
                "refusing to delete worktree at {}: it is not the worktree Mutex created \
                 (it was likely removed and recreated outside Mutex)",
                root.root_path.display()
            ))
        }
    }
...
pub async fn restore_worktree_via_git(
    ...
    if created_new_worktree {
        // Re-register the restored worktree as Mutex-created so it can be
...
mod tests {
        ...
        workspace.read_with(cx, |_workspace, cx| {
            ...
            assert!(
                ...
                "build_root_plan should return None for a linked worktree \
                 outside the Mutex-managed worktrees directory",
            );
        });
    }
    ...
    async fn test_build_root_plan_returns_none_for_unrecorded_linked_worktree_in_managed_directory(
        ...
        workspace.read_with(cx, |_workspace, cx| {
            ...
            assert!(
                plan.is_none(),
                "build_root_plan should return None for a linked worktree Mutex didn't create, \
                 even when it lives inside the Mutex-managed worktrees directory",
            );
        });
    }
    ...
    async fn test_build_root_plan_with_custom_worktree_directory(cx: &mut TestAppContext) {
        ...
        // It is recorded as Mutex-created so that the directory check, not
    ...
    async fn test_remove_root_refuses_when_worktree_recreated_outside_zed(cx: &mut TestAppContext) {
        ...
        // simulating a worktree that was removed and recreated outside Mutex
        // after Mutex recorded the original.
        ...
        assert!(
            error.to_string().contains("not the worktree Mutex created"),
    ...
    async fn test_remove_root_returns_error_and_rolls_back_on_remove_dir_failure(
        ...
        // worktree itself intact so the created-by-Mutex verification passes.
        let worktree_path = Path::new("/worktrees/project/feature/project");
