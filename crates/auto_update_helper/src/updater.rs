...
pub(crate) static JOBS: LazyLock<[Job; 22]> = LazyLock::new(|| {
    ...
    [
        ...
        Job::mkdir(p("old")),
        Job::move_file(p("Mutex.exe"), p("old\\Mutex.exe")),
        Job::mkdir(p("old\\bin")),
        Job::move_file(p("bin\\Mutex.exe"), p("old\\bin\\Mutex.exe")),
        ...
        // Copy new files
        Job::move_file(p("install\\Mutex.exe"), p("Mutex.exe")),
        Job::move_file(p("install\\bin\\Mutex.exe"), p("bin\\Mutex.exe")),
...
fn release_file_handles(app_dir: &Path) -> Result<()> {
    ...
    let files_to_release = [
        app_dir.join("Mutex.exe"),
        app_dir.join("bin\\Mutex.exe"),
...
pub(crate) fn perform_update(app_dir: &Path, hwnd: Option<isize>, launch: bool) -> Result<()> {
    ...
    if launch {
        ...
        let _ = std::process::Command::new(app_dir.join("Mutex.exe")).spawn();
    }
