...
impl LanguageModelProvider for CloudLanguageModelProvider {
    ...
    fn configuration_view_v2(
        ...
        // The Mutex sign-in/plan control is small enough that sending users to a
    ...
    fn authentication_error_message(&self) -> SharedString {
        "Failed to sign in with your Mutex account (401).".into()
    }

    fn missing_credentials_error_message(&self) -> SharedString {
        "You are not signed in to your Mutex account. \
        Sign in to continue."
            .into()
    }
    ...
    fn fast_mode_confirmation(&self, _cx: &App) -> Option<FastModeConfirmation> {
        Some(FastModeConfirmation {
            title: "Enable Fast Mode for Mutex?".into(),
...
impl RenderOnce for ZedAiConfiguration {
    fn render(self, _window: &mut Window, _cx: &mut App) -> impl IntoElement {
        let (subscription_text, has_paid_plan) = match self.plan {
            Some(Plan::ZedPro) => (
                "You have access to Mutex's hosted models through your Pro subscription.",
                true,
            ),
            Some(Plan::ZedProTrial) => (
                "You have access to Mutex's hosted models through your Pro trial.",
                false,
            ),
            Some(Plan::ZedStudent) => (
                "You have access to Mutex's hosted models through your Student subscription.",
                true,
            ),
            Some(Plan::ZedBusiness) => (
                if self.is_zed_model_provider_enabled {
                    "You have access to Mutex's hosted models through your organization."
                } else {
                    "Mutex's hosted models are disabled by your organization's configuration."
                },
                true,
            ),
            Some(Plan::ZedVip) => (
                "You have access to Mutex's hosted models through your VIP subscription.",
                true,
            ),

            Some(Plan::ZedFree) | None => (
                if self.eligible_for_trial {
                    "Subscribe for access to Mutex's hosted models. Start with a 14 day free trial."
                } else {
                    "Subscribe for access to Mutex's hosted models."
                },
                false,
            ),
        };
        ...
        if !self.is_connected {
            return v_flex()
                .gap_2()
                .child(Label::new("Sign in to have access to Mutex's complete agentic experience with hosted models."))
                .child(
                    Button::new("sign_in", "Sign In to use Mutex AI")
...
impl Component for ZedAiConfiguration {
    ...
    fn description() -> &'static str {
        "The configuration surface for Mutex's hosted AI models, \
        showing the user's connection status, current plan, trial eligibility, \
        and entry points for enabling the Mutex model provider."
    }
    ...
    fn preview(_window: &mut Window, _cx: &mut App) -> AnyElement {
        ...
        v_flex()
            ...
            .children(vec![
                ...
                single_example(
                    "Business Plan - Mutex models enabled",
                    configuration(PreviewConfiguration {
                ...
                single_example(
                    "Business Plan - Mutex models disabled",
                    configuration(PreviewConfiguration {
                        plan: Some(Plan::ZedBusiness),
                        is_connected: true,
