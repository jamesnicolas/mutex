...
/// same `RemoteConnectionIdentity`, matching how Mutex treats multiple
    ...
    let headless = server_cx.new(|cx| {
        remote_server::HeadlessProject::new(
            remote_server::HeadlessAppState {
                ...
                agent_turn_activity: remote_server::AgentTurnActivity::default(),
            },
            false,
            cx,
        )
...
async fn test_closing_last_agent_panel_terminal_restores_empty_header(cx: &mut TestAppContext) {
    ...
    assert_eq!(
        visible_entries_as_strings(&sidebar, cx),
        vec!["v [my-project]", "  New Mutex Agent Thread"]
    );
    assert_project_header_has_threads(&sidebar, "my-project", true, cx);
...
async fn test_remote_archive_thread_with_active_connection(
    ...
    // linked worktree across two remote projects the way Mutex does in
    ...
    // Record the worktree as Mutex-created on the client, keyed by the remote
