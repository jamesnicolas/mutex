...
Mutex uses select third-party subprocessors to deliver core product functionality. Each subprocessor processes customer personal data only as necessary to provide its service, and all are subject to appropriate data protection agreements.

### How Mutex Uses Subprocessors

To provide fast, reliable, and secure functionality, Mutex relies on a small number of carefully vetted third-party subprocessors. These vendors help us deliver essential capabilities such as hosting, billing, analytics, real-time collaboration, and hosted AI features.

Each subprocessor only processes customer personal data as needed to provide its service.

Mutex maintains contracts and data protection agreements with all subprocessors, including GDPR-compliant terms where applicable. We do not sell customer data, and we do not share customer personal data with vendors for advertising or marketing purposes.

### AI Subprocessors

Mutex offers three modes for AI:

1. **Bring your own API key** — data goes directly from the customer to the model provider; Mutex does not process or store it.
2. [**External Agents**](https://mutex.dev/docs/ai/external-agents) — Mutex uses ACP to provide an enhanced experience with terminal-based AI code agents like Claude Code or OpenAI Codex. Data is not processed or stored by Mutex when using external agents.
3. **Mutex-hosted models** — Mutex sends customer prompts to one of its AI providers (listed below). These vendors act as subprocessors only for customers who choose this mode.

### Ongoing Updates

**Last Updated**: March 2, 2026

This subprocessor list is reviewed regularly. Mutex will notify customers of material changes in accordance with our [Terms](https://mutex.dev/terms) and [Privacy Policy](https://mutex.dev/privacy-policy).
...
| **Fivetran**         | Automates data pipeline integration (extract, transformation, and load services) for Mutex | United States |
...
## AI Services (Mutex-Hosted Models)

_These subprocessors apply only when customers opt to use Mutex's hosted AI models. When users supply their own API keys, or use external agents, data is sent directly to the provider and does not pass through Mutex's infrastructure._
